...
**Q01 · 短链接服务** 〔L5〕— 设计一个日均 1 亿次跳转、峰谷比（peak-to-trough ratio）3× 的短链服务。
...
- **等这句**：「时钟回拨（clock rollback）是 Snowflake 唯一的正确性风险：回拨时要么阻塞等待、要么换 workerId、**绝不能继续发号**。如果我不需要跨机器严格有序，我直接用 UUIDv7 —— 它自带时间前缀，B-Tree 插入是顺序的，省掉一整套 workerId 分配基础设施。」
...
- **等这句**：「配置中心挂掉时**所有客户端必须能用本地快照继续跑** —— 否则你造了一个把 5000 个服务连成一体的单点。另外配置变更必须走和代码一样的灰度（progressive rollout）：**生产事故里配置错误的占比高于代码错误**，但绝大多数团队给配置的发布流程比代码宽松得多。」
...
- **等这句**：「延迟预算决定漏斗形状：召回 1000 条给粗排（每条算力 ~10µs），粗排留 100 条给精排（每条 ~1ms），精排出 20 条。**最容易出错的是在线-离线特征不一致（training–serving skew）** —— 训练用的是离线批算的特征，线上用的是实时算的，两者差一点，模型效果就会莫名其妙掉几个点。所以要么用同一份特征存储，要么做特征一致性监控。」
...
- **等这句**：「100ms 是**硬约束不是 SLO** —— 超时的请求价值为零，所以这里**不能重试、不能对冲（hedged request）**，只能降级到本地缓存的默认出价。整个系统的设计原则是'把所有远程调用消灭掉'：模型和特征全部内存驻留，任何需要跨网络取数的路径都必须有本地兜底值。」
...
- **等这句**：「**权限过滤必须在检索层做，不能在生成后过滤** —— 后过滤意味着模型已经看过它不该看的内容，可能通过改述泄露。混合检索（hybrid retrieval）是默认（dense 在错误码、SKU、函数名这类词法精确查询上系统性失效），但别指望开关一开就大涨：裸 RRF 相对纯 BM25 只有约 +1.3% NDCG，收益来自 alpha 调参和领域适配，大约需要 40 条标注对。」
...
- **等这句**：「**成本优化不要去抠沙箱**：Agent 成本里推理占 85–89%，沙箱运行时只占 11–15%，存储近乎 0（厂商官方算例，n=1）。杠杆在上下文长度、前缀缓存（prefix caching）命中率、子 Agent 的模型分级。另外**容器不是不可信代码的安全边界**（共享宿主内核），LLM 生成的代码必须跑在 microVM 里。」
...
- **考点**：编排 vs 编舞（orchestration vs choreography）｜子 Agent 的上下文防火墙与有损摘要｜并行只用于读｜扇出配额｜orchestrator 的认知视界
...
- **等这句**：「**单层限额一定不够**：单层硬限额会让大租户在月中直接停服，单层软限额挡不住失控 Agent 循环。必须双层 —— 软限额（soft limit）触发降级与告警，硬限额（hard limit）触发拒绝。Agent 场景还要加**单会话/单任务的绝对 token 上限**，因为 Agent 循环的方差比人类会话大一个数量级（多 Agent 系统的 token 消耗约是普通聊天的 15×）。」
...
- **等这句**：「**提示注入是架构问题不是可修补的 bug。** 已发表的 12 个防御在自适应攻击下多数 ASR > 90%，所以分类器和护栏只能当纵深，不能当边界。边界必须是确定性的：能力最小化、出口白名单、不可逆动作强制人工确认，而且**高影响动作清单要在设计期静态定义，不能运行时让 Agent 自己判断**。另外注意 MCP 的信任边界（trust boundary）在 **tool description 不在 tool 调用** —— Agent 只要列出了恶意 server 的工具，描述里的指令就已经进了 prompt。」
...
- **等这句**：「实测降本区间是 40–85%、保留 95–99% 质量，但**85% 是上限不是均值** —— 那来自偏易的基准且只有约 14% 请求走强模型。更重要的是**级联会抬高 P99**（小模型 + 大模型串行），所以路由必须做成**可一键下线的旁路（bypass）**，上线前先埋点统计'多少比例的请求小模型也能过'，用数据决定要不要做。」
...
- **考点**：逻辑分片（logical shard）与物理库的 1:N 映射｜双写与影子读（shadow read）的迁移六步法｜迁移期的一致性窗口｜限速与源库保护｜回滚点
