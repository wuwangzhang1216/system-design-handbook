...
| **LLM TTFT（首 token，中等模型）** | 200 ms – 2 s | 取决于 prompt 长度与是否命中前缀缓存（prefix caching） |
...
**Senior 的表述方式：**
> "分区期间，我们的支付路径选 CP —— 宁可返回 503 让客户端重试，也不能出现双花（double spend）。而通知路径选 AP —— 分区期间接受重复推送，靠客户端去重收敛。"
...
修复方式：`SELECT ... FOR UPDATE` 物化冲突（materializing conflicts）、显式约束、或用 SSI（PostgreSQL 的 `SERIALIZABLE`）。
...
| 去重表 | 幂等键 + 唯一索引 | 通用，推荐 |
| 版本/CAS | `UPDATE ... WHERE version = N` | 乐观并发（optimistic concurrency），同时解决 ABA |

---

## 5. 背压（Backpressure）：没有它系统就会雪崩（cascading failure）
...
**如果一个请求要扇出（fan-out）到 N 个后端，且必须等全部返回，那么整体的 p99 ≈ 单个后端的 p(99^(1/N))。**
