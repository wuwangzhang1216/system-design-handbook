...
> 这一条推导出后面所有规则。做不到这一条的团队，回滚按钮是假的。
...
**解耦这两件事是现代发布工程的地基。** 部署由工程按 CI 节奏做，发布由产品/运营按业务节奏做。它们解耦之后：
...
|---|---|---|---|
| 部署频率 | 按需，每天多次 | 每月~每半年 | |
| 变更前置时间 | < 1 天 | > 1 个月 | commit → 生产 |
| **变更失败率** | 5–15% | 40–60% | **最重要的一个**，它决定你敢不敢加速 |
| 恢复时间 | < 1 小时 | > 1 周 | 自动回滚能把它压到分钟级 |

> 如果变更失败率 > 30%，先别谈提高部署频率 —— 你只会更快地制造事故。先把金丝雀门禁和 schema 纪律做好。
...
|---|---|---|---|---|---|
| **Release toggle** | 隐藏未完成功能 / 渐进放量 | **天~周** | 部署或会话级 | 工程 | **发布完成后 2 个迭代内必须删** |
| **Experiment toggle** | A/B 实验 | 天~周 | 每请求（按用户稳定分桶） | 数据/产品 | 实验结束即删 |
| **Ops toggle / kill switch** | 降级、熔断、关掉重功能 | **月~永久** | 运行时立即生效 | 值班工程师 | 长期保留，但要定期演练 |
...
4. 死代码检测：flag 已 100% 打开且稳定 30 天 → 自动生成"删除分支"的 PR。
...
**一次请求内必须求值一次**：在入口（网关/BFF）求值，把结果放进请求上下文往下传。让每个下游服务各自求值，会因为规则集推送延迟（通常 1–30 秒）导致**同一请求内前后不一致** —— 服务 A 认为开了、服务 B 认为关了，产生的数据状态是任何一个版本的代码都没预期过的。

**性能与故障模式**：
- SDK 必须**本地求值**：规则集推送到进程内，求值 < 1 µs。**绝不在请求路径上做网络调用**。
...
2. **生效时间 < 30 秒**：推送（SSE/长轮询）而非 5 分钟轮询。事故中的 5 分钟是很长的时间。
3. **定期演练**：每季度真的拉一次。没演练过的 kill switch 在事故当天有很高概率是坏的。
...
|---|---|---|---|---|
| **金丝雀** | 秒~分钟 | +5% | 真实流量下的错误率、延迟、资源 | 低频路径、长尾租户、需要数小时才显现的泄漏 |
| **蓝绿** | 秒（切流） | **2×** | 全量切换的兼容性 | 渐进信号（要么全好要么全坏） |
| **影子流量** | N/A（不影响用户） | +1× 计算 | 性能、响应差异 | **写路径**（有副作用，除非做完整的副作用隔离） |
| **Ring / 按租户放量** | 分钟~天 | 低 | 租户维度的爆炸半径、企业客户的定制路径 | 需要租户路由能力 |
...
- [ ] 有**并发运行的基线组**（同时段的老版本流量），**不是**"和昨天同一时刻比"
- [ ] 样本量门槛：canary 累计 ≥ **1,000 个请求**且 ≥ **5 分钟**，两者取后到者；不满足就不判定
- [ ] 回滚动作本身经过演练，且回滚后**自动冻结流水线**（否则下一个人会把同样的变更再推一遍）
...
| 3 | **新错误指纹** | 出现 baseline 中不存在的 exception 堆栈指纹 | **立即** | 最灵敏的信号 |
| 4 | 下游依赖错误率 | > baseline × **2** | 5 min | 抓"新版本把下游打挂了" |
| 5 | 饱和度 | CPU / 内存 / 连接池 / 队列深度 > **85%** | 5 min | 抓资源泄漏 |
| 6 | **业务 KPI** | 关键转化率 < baseline × **0.95** | 15 min | 抓"技术指标全绿但功能坏了" |
| 7 | SLO 燃尽率 | 1 小时窗口 burn rate > **14.4** | 立即 | 见 [05-reliability/01](../05-reliability/01-slo-and-error-budget.md) |
...
**这是发布工程里最容易出人命的一节。** 根本原因：滚动发布期间新旧代码同时在跑，数据库必须同时对两者合法。
...
| **3** | **Backfill** | 分批回填历史数据（**幂等 + 可重跑 + 限速**） | 写双，读老 | ✅ 新列是纯冗余 | 复制延迟上升、锁等待、批次超时 | 小时~天 |
| **4** | **Verify** | 影子读 + 逐行 diff（抽样或全量） | 写双，读老，**旁路比对新** | ✅ 完全可回滚 | diff 率不收敛到 0 | 1~7 天 |
...
> `lock_timeout = '2s'` + 外层指数退避重试是唯一正确的做法。这一条比本节其他所有内容加起来更常救命。

**MySQL 侧**：`ALGORITHM=INSTANT` 覆盖加列等一部分操作；其余用 [gh-ost](https://github.com/github/gh-ost)（基于 binlog，无触发器，**可暂停、可限流、可随时中止**）或 `pt-online-schema-change`（基于触发器，写放大更大）。选 gh-ost。
...
| **改字段语义**（单位从"分"变"元"） | ☠️ **最危险** | 编译期、schema 校验、契约测试**全都发现不了**。改语义必须改名字。 |
...
- **契约测试**：消费者驱动的契约（Pact 类）放进 CI，让"我改了一个字段"在合并前就被下游的契约挡住。
...
2. **`version` 作为一等指标维度**：所有 RED 指标（Rate/Error/Duration）带 `version` 标签，这样金丝雀对比是一次查询而不是一个项目。
3. **告警自动关联最近变更**：告警触发时，自动附上"过去 60 分钟内该服务及其直接依赖的所有部署与 flag 变更"。这一条能把 MTTR 砍掉一半。

**自动化的变更前后报告**：每次部署完成后 30 分钟自动生成 diff 报告（错误率、p50/p99、成本、关键 KPI），推到部署 PR 的评论里。它的价值不在于告警（金丝雀已经做了），而在于**让缓慢劣化被看见**。
...
Anthropic 的 prompt cache **失效级联顺序是 `tools → system → messages`**：
...
**工程结论**：
- 工具定义变更要**错峰发布**（低谷期），并盯住 cache hit rate 指标直到恢复。
...
| 要求 | 数值 | 来源/理由 |
|---|---|---|
| CI eval 数据集规模 | **100+** 精选样本 | 来自**生产轨迹**，不是公开 benchmark |
| 初次错误分析 | **20–50 条** trace；后续每轮 **≥100 条** | 饱和判据：连续约 20 条无新失败类别 |
| 错误分析节奏 | 活跃期每 **2–4 周**一轮；间隔期每周看 10–20 条 | |
| 失败模式分类法规模 | **5–10 类**；手工 open-code 30–50 条后再让 LLM 辅助聚类 | |
...
⚠️ **judge 有系统性偏差，且换 judge = 换基线**。已观测到的量级：**风格偏差 0.10–0.76，位置偏差 ≤0.04**（风格偏差是位置偏差的十倍以上）；冗长偏好方向**因模型而异**（部分模型偏好长回答 +0.24~+0.44，Claude 系偏好简洁 −0.12）。⇒ judge 模型也要写进 release manifest，换 judge 必须重新校准，**不同 judge 的分数不可横向比较**。

### 7.4 模型下线与迁移

**换模型不是换一个字符串。** 一次模型迁移至少要重算四件事：

1. **Tokenizer 变了** —— Claude 4.7 及以后（Opus 4.7/4.8/5、Sonnet 5、Fable 5）同样文本约多产生 **+30% token**。你的上下文预算、截断阈值、成本模型、限流阈值全部要重算。
...
**纪律**：`model` 字段一律写完整 snapshot ID。用 `latest` 别名等于把"发布"这个动作外包给了供应商 —— 你会在某个周三凌晨经历一次你没做的发布。把每个模型的 EOL 日期当作**有到期日的技术债**跟踪，和证书过期一样对待。

### 7.5 A/B 与在线评测

- **分流单位是会话/任务，不是请求。** 同一会话必须 sticky，否则：(a) 前缀缓存全废（成本与 TTFT 双恶化），(b) 用户看到人格分裂的行为，(c) 指标被污染。
- **LLM 的方差比传统服务大**：同一输入多次采样结果不同 ⇒ 需要的样本量显著更大，1% 流量的小实验通常得不出结论。先算功效（power），再开实验。
- **影子流量对 LLM 是成本翻倍**：必须抽样（1–5%），且只影子**读路径**（有工具副作用的路径不能影子）。
- **在线指标优先于离线分数**：任务完成率、人工干预率、重试率、会话轮数中位数、**每任务成本**。离线分数用来挡住明显退化，在线指标用来决定是否全量。
...
| **变更冻结窗口滥用**（"季度末不许发布"） | 冻结期结束时积压一大批变更同时上线 —— 风险不是消失了，是被压缩了 | 只冻结高风险类别，且冻结期照常做小批量发布 |
