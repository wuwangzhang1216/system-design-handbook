...
> 把地基上的核心词真正讲清楚（包括负载均衡在那条链路上到底在干什么），因为后续章节会直接复用这些概念。

**这本书有两种用法。先选一种，不要把两套顺序混在一起：**

| | 当**教材**顺读 | 当**手册**查阅 |
|---|---|---|
| 入口 | [00-foundations/00-concepts.md](00-foundations/00-concepts.md) → [知识地图](00-foundations/README.md) 的**概念依赖路径**。如果目标是近期面试，直接从 [START-HERE.md](START-HERE.md) 自测开始，由结果决定要补多少教材主线 | 下面的 [目录](#目录) ｜ [07/03 速查表](07-interview/03-cheatsheet.md) ｜ [07/04 术语表](07-interview/04-glossary.md) |
| 顺序 | 按知识地图补齐前置；不需要“先通读整本教材，再重复做 Path A”。START-HERE 会为了面试收益打散章节，少数诊断性检查点会故意让你先尝试、再补深层前置，并明确标出 | 没有全局顺序，按主题跳；每篇开头的前置清单负责告诉你缺什么 |
| 适合 | 第一次系统学、或自测发现地基有洞 | 已经有工程经验、临时查一个决策或一个数字 |

**唯一一条硬规则**：无论哪种用法，遇到读不下去的段落，先回 [00-foundations/README.md 知识地图](00-foundations/README.md) 查这个词首次定义在哪一节。若术语都认识但专项细节仍然过陡，按 START-HERE 当天指定的小节继续；岗位专项内容可以先跳过。

### 五个入口文件，各管一件事

| 文件 | 回答什么问题 | 什么时候打开 |
|---|---|---|
| **[00-foundations/00-concepts.md](00-foundations/00-concepts.md)** | 这些词到底是什么意思？ | 术语不稳，或入口自测只有 0–2 题时 |
| **[00-foundations/README.md](00-foundations/README.md)** | 哪个概念依赖哪个概念？ | 第一次系统学习，以及读到一半卡住时 |
| **[START-HERE.md](START-HERE.md)** | 我今天该读哪一小节、练什么？ | 准备面试时；它按收益排，不是目录顺序 |
| **[PRACTICE.md](PRACTICE.md)** | 我该怎么计时、录音和复盘？ | 开始练习前配置一次，之后反复使用 |
| **README.md**（本文件） | 书里有什么、某个主题在哪？ | 查阅时 |

全书用四种**导航语义**：**Core** 是通用 Full Stack 主线；**Deep Dive** 是同一主题的可选专项内核；**Specialty** 仅目标岗位需要；**Snapshot（截至 YYYY-MM）** 是会随厂商、价格、法规或版本变化的实现快照。它们不是每篇都重复展示的强制徽章：日常学习以 START-HERE 指定小节为准。尤其要区分两件事：案例里的“**深挖**”是 45 分钟面试的必答阶段；只有明确标成英文 **Deep Dive** 的内容才表示可选专项。

## 先选一条：你为什么打开这个仓库

| 你的情况 | 去哪 |
|---|---|
| 😵 **看不懂 / 术语一头雾水** | → **[00-foundations/00-concepts.md](00-foundations/00-concepts.md)** —— 全书唯一一篇从请求链路开始建立词典的文章。p99、分片、最终一致、幂等这些词在这里首次系统解释。**先花 2–3 天读完它再谈路径** |
| 🎯 **我是来准备面试的** | → **[START-HERE.md](START-HERE.md)** —— 10 分钟自测 → 按分数选路径 → 31 天逐日计划。**别从这个 README 往下读**，那是查阅顺序不是学习顺序 |
...
|---|---|---|---|
| **通用后端 / 全栈**（默认） | — | [A](START-HERE.md#3-路径-a四模块--31-天逐日计划) / [B](START-HERE.md#4-路径-b10-天冲刺) | 电商、支付、SaaS、基础设施、平台组 |
| **AI Agent / LLM 应用** | [04](04-ai-agent-systems/)（8 篇） | [C](START-HERE.md#5-路径-cai-agent--llm-应用岗的额外-2-周) | JD 里有「LLM / Agent / RAG / prompt / GPU 推理」 |
...
> | 常见延迟侧重 | 交互式生成通常为百毫秒到秒级 | 在线排序常为毫秒到百毫秒级；批任务不在此范围 |
> | 常见质量信号 | eval 集、人工/模型评审，也可能有延迟标签 | 离线指标、实验与线上标签；也可能面对延迟或缺失标签 |
>
> 同时服务 LLM 与传统模型的推理平台岗通常需要两章都读；其余情况优先按 JD 选一章。
> **通用面试路径（路径 A / B）默认跳过两章主体**。缓存、队列、限流、SLO 等通用机制已经在 00–05 讲过；只有 JD 明确涉及 AI/ML 时才值得投入专项篇幅。

**中英双语**：正文里关键概念首次出现时会跟上英文原词；
`07/04` 把它们汇成主题化对照表，`07/05` 是配套英文话术库，`07/06` 是一场英文模拟面试全逐字稿。
...
| [00-concepts.md](00-foundations/00-concepts.md) | **全书唯一一篇从请求链路开始建立词典的文章。** 请求的一生、延迟/吞吐/并发、p99、扩展、副本与分片、一致性、事务与隔离级别、同步异步、有无状态、可用性、缓存/队列/索引。**这一篇必须按顺序读**，后文会直接复用这些概念 |
...
> 面 AI 产品 / LLM 网关 / Agent 基础设施 → 这 8 篇是你的主场，走 [START-HERE 路径 C](START-HERE.md#5-路径-cai-agent--llm-应用岗的额外-2-周)。
...
前 9 题可归约到 **4 种高频母题**：
**A 唯一性 · B 扇出与聚合 · C 并发与库存 · D 长连接与状态**；第 10 题对象存储刻意保留为“控制面与字节数据面分离”的边界案例。认出复用模式比背题重要，也要知道分类何时不适用。
...
| 文件 | 用途 |
|---|---|
| [07-classic-canon.md](06-case-studies/07-classic-canon.md) | **A 组 10 题的压缩速查版**（不能替代展开版）。前 9 题归约到 4 种母题，对象存储作为边界案例。两个用法：① 开局扫一遍摸底，再去读对应展开版；② 面试前一天检查模式识别与边界判断 |
...
| 文件 | 案例 | 这题的枢纽 |
|---|---|---|
| [18-model-serving-platform.md](06-case-studies/18-model-serving-platform.md) | 模型服务平台 —— 300 个模型共享 GPU 池 | 调度同时受显存、计算时间与互联约束；负载不能只按请求数计 |
...
| [04-glossary.md](07-interview/04-glossary.md) | 按主题组织的中英术语对照（含复制模式与 ML 系统/实验）；给出可直接嵌进回答的整句，并标出直译易错词、成块使用的行话和易错重音 |
...
| 路径 | 适合谁 | 时长 | 04 章 | 08 章 |
|---|---|---|---|---|
| **[A · 通用 full-stack / 后端](START-HERE.md#3-路径-a四模块--31-天逐日计划)**（默认） | 大多数人。面 senior 后端 / 全栈 / 平台岗 | 4 个模块 / 31 个学习日（多数天 60–90 min；重日已标注，可拆分） | ✕ 跳过 | ✕ 跳过 |
| **[B · 10 天冲刺](START-HERE.md#4-路径-b10-天冲刺)** | 面试在 10 天内，或自测答对 10+ | 10 天（每天 90–120 min） | ✕ 跳过 | ✕ 跳过 |
| **[C · AI Agent / LLM 应用岗](START-HERE.md#5-路径-cai-agent--llm-应用岗的额外-2-周)** | 目标是 AI 产品 / LLM 网关 / Agent 基础设施 | 43 个学习日（每天学习约 6 周） | ✓ 主体 | ✕ 跳过 |
...
> **F 不是 A 的续集，是一条独立的 6 周路径**：它把路径 A 四个模块里 ML 岗用不上的部分（多租户、SaaS 平台、协作编辑）砍掉，
...
时间只够四组内容的话，读 [08-ml-systems/README.md §5](08-ml-systems/README.md#5-怎么读这一章) 那一行「一周内面试」—— 它直接覆盖 ML 组 19 项里的 7 项，并补到训练/服务偏斜与多阶段延迟预算；准确口径以 08 章入口表为准。
...
五个入口的职责已经放在本页最前面。学习时只保留两个标签页：**知识地图**负责前置依赖，**START-HERE** 负责当天任务；其余文件按需查阅。
