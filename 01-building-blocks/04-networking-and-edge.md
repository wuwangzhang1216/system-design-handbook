...
| 概念 | 一句话 | 详见 |
|---|---|---|
| 一个请求的旅程 | DNS → CDN → 负载均衡器 → 服务，每一跳在干什么、挂了会怎样 | [00-concepts §1](../00-foundations/00-concepts.md#1-一个请求到底经历了什么) |
| Little's Law | 并发 = 吞吐 × 延迟；连接池和线程池的大小都从这里反推 | [00-concepts §2](../00-foundations/00-concepts.md#2-延迟吞吐并发--三个最常被混淆的词) |
| 有状态 vs 无状态 | 替换任意实例时，正确处理请求是否依赖该实例独有的先前上下文；持久数据与可恢复会话应外置 | [00-concepts §9](../00-foundations/00-concepts.md#9-有状态-vs-无状态) |
| 强一致 / 最终一致 | 有多个副本时"我读到的是不是最新的" | [00-concepts §6](../00-foundations/00-concepts.md#6-什么是一致性--一个词两种完全不同的意思) |
| RTT 与光速下限 | 跨洲一个来回至少 80–150 ms，这部分花钱也买不回来 | [01-fundamentals §1](../00-foundations/01-fundamentals.md#1-为什么要有这些数字) |
| 超时 / 重试 / deadline 传播 | 每层只减不加，重试只在一处做，否则 3 层 ×3 次 = 27 倍放大 | [01-fundamentals §9](../00-foundations/01-fundamentals.md#9-幂等--重试--超时三件套必须一起设计) |
...
| 二选一取优 | power of two choices (P2C) | 随机挑两个后端，用负载信号选择较轻者；不需要集中维护全局精确状态，但仍依赖采样到的局部状态 |
...
> 这是 **LLM 网关做会话粘性（session affinity / sticky session）路由的正确算法** —— 既保持 KV cache（LLM 推理时为已处理过的 token 缓存下来的中间状态，同一会话落回同一实例才能复用，见 [`02 §7`](02-caching.md#7-专项选读llm-缓存层)）亲和性，又不会让某个实例过载。
...
| **Power of Two Choices (P2C)** | 随机选 2 个，用本地可见的负载信号取较轻者；避免集中维护全局精确状态 |

**P2C 值得单独记住**：精确的全局最少负载需要汇总全局状态，可能昂贵或过时；随机采两个再比较，在许多负载模型下能以很低协调成本取得好效果。它不是“完全无状态”——每个选择仍依赖采样的活跃请求、连接或延迟信号，效果也要用真实负载验证。

**专项提示（Agent/LLM）**：当请求成本差异很大时，Round Robin 容易失衡。可以比较 least-outstanding-requests、排队 token/计算量、延迟感知与带负载上限的亲和路由；没有一个算法在所有模型与批处理策略下都最优。
...
| 参数 | 怎么定 |
|---|---|
| 最大连接数 | Little's Law 用稳定窗口的平均 QPS × 平均连接占用时间给出平均并发起点；上限还受下游容量、尾延迟、突发和排队策略约束，没有通用 2× 余量 |
...
这就是**队头阻塞**在传输层的形态：排在前面的那个包没到，后面已经到了的包也不许交给应用（同一个词在 Kafka 分区消费上的形态见 [`03 §1`](03-messaging-and-streams.md#1-队列-vs-日志两种根本不同的东西)）。
...
问题 3：连接处理层有实例本地状态，会影响扩缩容
  → 扩容时新实例没有连接，缩容时要迁移连接
  → 若业务会话与 KV cache 也只存在本机，故障影响会进一步扩大
  → 解法：持久会话/续传点外置 + 断线可恢复；连接网关可以是有状态层，而后端业务 API 仍可保持可替换的无状态语义
```
...
2. **CRDT / 最后写入者胜**：适合可交换的数据（计数器、集合、协作文档）。CRDT 是一类特殊设计的数据结构 —— 多个副本各自乱序合并后必然收敛到同一个结果，因此完全不需要协调；"最后写入者胜"（LWW）则依赖时钟，为什么危险见 [`05 §5`](05-consensus-and-coordination.md#5-时间分布式系统里最不可信的东西)。
...
**边缘的真实约束**：边缘运行时通常有 CPU、内存、执行时间、网络与本地存储限制，且不同套餐会变化。状态往往要放到提供商的持久服务或源站；选型时查当前配额并测试冷启动与跨区访问。

**专项提示（LLM）**：边缘适合做路由、鉴权、限流与缓存查找；推理通常放在具备所需加速器和模型权重的区域集群。小模型或提供商支持的边缘推理是例外，不能把“必须在中心 GPU”当成协议约束。
...
| 路径 | 计费检查点 |
|---|---|
| 同 AZ 内 | 不是所有服务/路径都免费；检查源和目的服务的当前价目 |
| **跨 AZ** | 常按一个或两个方向计费，规则随服务与地域变化 |
| 跨 Region | 检查两端地域、复制服务和方向 |
| **出网到 Internet / CDN** | 按地域、阶梯、套餐、免费额度和协商折扣变化 |
...
3. **替代对象存储的出网政策可能更低**，但要同时比较请求费、生态、可靠性、迁移与当前条款

**估算方法示例**：月费 = 每日跨域 GB × 天数 × 当前每 GB 单价 × 被计费方向数。再比较压缩前后实测字节量；不要把某个历史价目或“能省 80%”当作所有系统的结果。
...
**没有超时 = 无限期挂起 = 线程/连接泄漏 = 级联故障（cascading failure：一个组件变慢导致上游并发堆积、资源耗尽，故障沿调用链一路向上传染，见 [`01-fundamentals §6`](../00-foundations/01-fundamentals.md#6-背压backpressure没有它系统就会雪崩cascading-failure)）。** 很多语言的 HTTP 客户端默认无超时，这是生产事故的头号来源之一。
...
2. **长连接让连接处理层具有实例本地状态，但不必让整个业务服务都变成不可替换。** 把可恢复会话、续传点和业务状态外置后，网关仍需 drain 与重连设计，后端 API 可以保持无状态。SSE 的优势之一是协议定义了 `Last-Event-ID`；服务端仍要持久化或重建对应事件。
...
**目录顺读下一篇** → [05-consensus-and-coordination.md](05-consensus-and-coordination.md)
