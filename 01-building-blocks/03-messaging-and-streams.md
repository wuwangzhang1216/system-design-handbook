...
> 引入队列不是"加个组件"，是把你的系统从"同步的、可调试的"变成"异步的、最终一致的、需要幂等和死信处理的"。这个代价必须自觉支付。
...
| 重放 | 不可（除非 DLQ） | **可以，任意回到过去某个 offset** |
| 多消费者 | 竞争消费（每条消息给一个） | 每个消费组独立读全量 |
...
**经典错误**：用 Kafka 做任务队列。
> 一个慢任务会阻塞整个分区（因为 offset 是单调的，不能跳过），导致后面所有消息延迟。任务队列需要"单条消息独立 ack/nack"，这正是 Kafka 不提供的。

**反过来的经典错误**：用 SQS 做事件总线。事件需要被多个下游独立消费且能重放，SQS 的删除语义做不到（要给每个消费者一个队列 + SNS 扇出，运维复杂且无法重放历史）。
...
**分区数只能增不能减**，且增加会破坏 key → 分区的映射（已有数据不迁移）→ **顺序保证被打破**。所以初始规划要留余量。

**b) 顺序保证的边界**
> Kafka 只保证**同一分区内**有序。同一个 key 通过 `hash(key) % partitions` 落到同一分区 → **同一实体的事件有序**。
> 跨分区无序。所以"全局有序"要么单分区（吞吐 = 单分区上限），要么在消费端重排（需要版本号/时间戳）。
...
- 静态成员（`group.instance.id`）避免滚动重启触发 rebalance

---

## 3. "Exactly-Once" 的真相

**残酷事实**：在分布式系统中，端到端的 exactly-once **消息传递**是不可能的（两将军问题）。
...
**面试标准答案**：
> "我不会说系统是 exactly-once。我会说：传递是 at-least-once，处理是幂等的，因此**效果**上是 exactly-once。具体做法是消费端用 message_id 做去重表，去重记录和业务写入在同一个数据库事务里提交。"
...
| 错误类型 | 处理 |
|---|---|
| **瞬时错误**（网络、下游 503、锁冲突） | 重试，指数退避 + 抖动 |
| **永久错误**（消息格式错、业务规则拒绝） | **立即进 DLQ，不要重试** |
| **毒丸消息**（导致消费者崩溃） | 必须有重试上限，否则消费者无限重启 |
...
**推荐**：默认用 **2（携带状态）**，因为它消除了下游对上游的同步依赖 —— 这才是解耦的真正价值。载荷大不是问题（压缩后通常几 KB）。
...
| 只有一个消费者且逻辑简单 | 直接同步调用，用重试 + 熔断 |
| 团队没有 DLQ / 幂等 / 监控的准备 | 队列会把 bug 变成"消息神秘消失" |
| 只是为了"削峰" 但峰值只有 2× | 加机器更便宜也更简单 |
...
3. 写数据库 + 发消息怎么保证原子性？
4. 一条毒丸消息会怎么样？怎么防？
5. 消费者处理很慢，消息堆积了 1 亿条，怎么办？（→ 加分区+消费者 / 跳过历史 / 并行处理 / 先分流再处理）
