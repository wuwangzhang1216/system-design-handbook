...
**如果你是直接翻到这道题的**：这题的支点是"接入层是有状态的"。第 1 题答不出，你会按无状态 Web 服务来设计它，后面关于路由表、租约和序号的每一节都会读成过度设计。
...
| 构件 | 用在哪 | 详见 |
|---|---|---|
| 有状态 vs 无状态、有状态路由 | §4.1 `uid → 哪台网关` 这张映射表存在哪，是整道题的支点 | [`00-foundations/00-concepts.md`](../00-foundations/00-concepts.md) §9 |
| 长连接管理、WebSocket / SSE、QUIC connection migration | §2.1 500 台接入机；§4.1 单机 10 万连接的约束 | [`01-building-blocks/04-networking-and-edge.md`](../01-building-blocks/04-networking-and-edge.md) §2 §4 |
| 租约（lease）、fencing token、时钟不可信 | §4.1 两级租约把 166.7 万写/s 降到 50/s；`conn_epoch` | [`01-building-blocks/05-consensus-and-coordination.md`](../01-building-blocks/05-consensus-and-coordination.md) §4 §5 |
| 幂等键、至少一次投递、失败模型 | §3 `client_msg_id` 去重；§4.4 推送与 socket 谁先到都不能重复展示 | [`00-foundations/01-fundamentals.md`](../00-foundations/01-fundamentals.md) §5 §8 |
| 退避与抖动、重试放大、优雅降级 | §5 重连风暴（瓶颈是 TLS 握手 CPU）；Registry 挂掉时降级不降正确性 | [`05-reliability/03-resilience-patterns.md`](../05-reliability/03-resilience-patterns.md) §3 §7 |

**这道题的一句话本质**

> **推送管延迟，拉取管正确性 —— 唯一能证明"消息送到了"的，是客户端带着 `since_seq` 自己回来拉。**
> 把整条推送链路删掉，系统依然正确，只是慢。而这条拉取线之所以成立，全靠会话内那个单调且**连续**的序号；连续（而不只是有序）才让客户端能自己证明"我没漏"，也正因如此，撤回永远不能物理删掉一个 seq。
...
- **第 2 题（群上限）。** 10 人群和 10 万人群不是同一个系统。前者写扩散（fan-out on write：发消息时就给每个成员写一条"你这个会话有新消息"的索引，读时 O(1)；反面是读扩散 fan-out on read —— 不写索引，成员打开会话时自己来拉）无脑正确，后者会产生比消息本身多三个数量级的索引（§4.6 给数字）。按 10 人群画完整张图，面试官一句"那 10 万人的群呢"就能把方案清零。
...
**推论 1：接入层的瓶颈既不是内存也不是稳态 CPU，是重连时的 TLS 握手**（每建一条加密连接都要做一次非对称密码运算，是这一层唯一的 CPU 大头；连接建好之后的收发只是便宜的对称加密）。
...
```
 ...
 └─────────▲───────────────┘                  │ → APNs / FCM      │
           │  GET /sync?since=S               └───────────────────┘
           │        （APNs / FCM：Apple 与 Google 的系统级推送通道，
           │          App 已被系统杀死时也能把设备唤醒）
...
> **水位线（watermark）**：一个"我已经处理到这里了"的游标，只允许单调前进、不允许回退。它的粒度（每设备一个？每会话一个？每设备每会话一个？）直接决定要存多少行。
