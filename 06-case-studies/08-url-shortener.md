...
| 4 | 链接会过期或被删除吗？短码能复用吗？ | 支持 TTL；**不复用** | 决定墓碑（tombstone）策略与回收成本（§4.6） |
...
读路径（3,500 QPS 峰值，要求 p99 < 30 ms，必须多区域）
 Client ──▶ Anycast ──▶ Edge Worker
                          ├─ ① 撤回名单（deny list，全量常驻，TTL 10 s）→ 命中则 410
                          ├─ ② 边缘 KV：code → {url, expire_at}   ≈ 92% 命中
                          │      └─ 命中 → 302 + Location，并 fire-and-forget 一条点击事件
                          └─ ③ 未命中 ──▶ 回源（origin fetch）
                                            L1 进程内 LRU（1 万条 / TTL 5 s）≈ 60% 命中
                                            └─▶ Redis 集群 ≈ 95% 命中
                                                  └─▶ Postgres（回源信号量限并发 200）
...
| **D. 哈希截断（MD5/SHA 前 7 位）** | 生日碰撞：10 年内约 **189 万次**冲突，单次 0.104% | 同 B，必须检查 | ✅ | 无界 | 同 B |
...
第二步 · 置换（消灭可枚举性），把自增变成不可预测但仍双射的整数
  balanced Feistel network，4 轮，21 位半块 → 域 2⁴²
    ...
    out = (L << 21) | R
  2⁴² = 4.398e12 > 62⁷ = 3.52e12 ⇒ 落在 [62⁷, 2⁴²) 的输出做 cycle-walking 再置换一次
...
| **边缘内存环形缓冲 + 批量 flush（100 ms 或 1,000 条）** | **0 ms**（fire-and-forget） | 崩溃丢最后一批 ≈ 350 条 | **零依赖** | ✅ |
...
2. **独立访客用 HyperLogLog，但只对头部开。** 一个 dense HLL 是 12 KB、标准误差 0.81%。500 万个活跃码全开 = 60 GB，不可接受；只给点击量 Top 1%（5 万条）开 = 600 MB，其余用稀疏表示或干脆不提供。**"精确到个位的独立访客"是一个没人愿意为之付 100 倍成本的需求。**
...
| 压缩版 | [`06-case-studies/07-classic-canon.md`](07-classic-canon.md) §1 | 同一题的 6 分钟版本 |
