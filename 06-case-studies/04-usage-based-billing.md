...
> 计费系统的正确性目标是**不对称**的：漏计只是亏钱，重计是客诉、退款和法律风险。
...
| 误差预算是多少？漏计和重计**分别**能容忍多少？ | **这是整道题的地基**，见 §3 |
| 账期按自然月还是订阅锚点日？租户时区？ | 决定聚合边界与月末峰值 |
| 需要实时硬限额吗？延迟预算多少？ | 决定要不要做本地令牌 + 周期同步 |
| 账期关闭后的迟到事件怎么处理？ | **必须写进产品条款，三选一**，见 §6 |
| 争议窗口 12 个月还是 24 个月？ | 决定原始事件保留期与审计粒度 |
...
⇒ **必须在自己这一侧预聚合**，原始事件留在自己的数据湖里。

**推论 2**：原始事件的存储成本是 **$8/月**。
⇒ **没有任何理由不永久保留原始事件。** 它是重算、对账、争议处理的唯一依据。删它省下的钱，一次客户争议就赔光。
...
**为什么不对称**：丢事件 = 少收钱，方向**永远对供应商不利** —— $5,000 万 ARR 上 0.5% 的事件丢失 = **$25 万/年**。而重复计费是客户看到账单翻倍 → 工单 → 退款，在受监管市场可能是合规事件。

> **面试金句**
> "我会给漏计和重计设两个差一个数量级的预算，并且用**两种完全不同的机制**保证：漏计靠 outbox + at-least-once 保证'一定送到'，重计靠业务语义派生的幂等键 + 去重窗口保证'不会算两次'。试图用一个 exactly-once 同时解决两个问题的系统，最后两个都保证不了。"
...
1. **计量与业务写路径解耦**。API 层只负责可靠投递到 log。**绝不在同步请求路径做去重查询** —— 那会把计费系统的可用性绑进产品可用性。
...
1. **尽力投递**：采样、缓冲区满即丢、磁盘满、agent OOM，没有一环承诺不丢。
...
去重的事实标准是 CloudEvents 的 `source` + `id`。OpenMeter / Lago / Orb / Metronome / Stripe 都以幂等键为前提，差异在**窗口长度、是否接受迟到事件、是否支持回填重算** —— 这几点公开文档披露不足，**选型时必须实测**。去重键格式、"最终一致去重"的行业惯例与那个致命误解的完整论证，见 [`03-saas-platform/02-billing-and-metering.md §4`](../03-saas-platform/02-billing-and-metering.md)。
...
**更省的做法**：按 `(tenant_id, hour_bucket)` 分桶去重，窗口压到 7 天，越界事件进**异常队列**而非静默丢弃或静默接受。异常队列必须有人看 —— 它是"客户端重试逻辑坏了"的唯一信号。

### 分区与热点

Kafka 按 `tenant_id` 分区 ⇒ 去重可用本地状态。但巨型租户会打爆单分区。二级分区：
```
...
**账期关闭必须有明确的"关闭时刻"和"关闭后不可变"语义**：finalize 时把该账期聚合结果快照进不可变表（`usage_snapshots`），发票行引用快照 ID。之后无论上游怎么重算，这张发票的依据不变。
...
**允许的漂移**：与 provider < 0.5%（存在跨时区归属、舍入、失败请求是否计费的口径差异）；与应用侧 < 0.05%（同一份事实的两条路径，漂移大就是有 bug）。
...
| 多币种 | 按租户结算币种 | **汇率必须在出账时刻快照写进发票行，永不重算** |
...
嵌入式脚本（Lua/JS）表达定价 = 不可审计、不可静态校验、不可确定性重放。用声明式价格计划：
...
2. **评级是纯函数**：`rate(usage_snapshot, plan_version, fx_snapshot) → line_items`。同输入永远同输出，可离线重放。
3. **金额用定点数**（`NUMERIC(20,10)` 或整数最小货币单位），**绝不用 float**；舍入规则（方向、在哪一步舍）写死并测试。
...
COMMIT;
```
**发票号必须由不可跳号的序列生成**（多数税务辖区的硬要求）⇒ 号码在 finalize 阶段才分配，不能在可能回滚的事务里浪费。
...
> **面试金句**
> "我不追求配额强一致，我追求**超支上界可计算**。20 个实例 × 单次租借 1 万 token = 20 万 token 的理论超支，按当前单价折合 $1。我把这个数字写进设计文档，让业务方决定它可不可接受；不可接受就调小 MAX_LEASE，用更多中心调用换更小上界 —— 这是一个连续旋钮，不是一个是非题。"
...
**别指望用量计费能修复毛利。** AI 产品平均毛利约 **52%**（传统成熟 SaaS 基准 80–90%，ICONIQ 2026 口径），而这个数字是在大量公司**已经转向用量计费之后**测出来的。真正拉毛利的是模型路由、缓存、蒸馏和规模效应（见 `../04-ai-agent-systems/08-cost-and-latency.md`）；定价只是把成本方差转移给客户。
...
| 时钟漂移导致事件落错桶 | 跨桶漂移在午夜集中 | `event_time` 由服务端赋值（不信客户端时间）+ NTP 监控 |
