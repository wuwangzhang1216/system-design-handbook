...
> 漏斗不是省钱手段，是一条守恒律：**每一级的「候选数 × 单条算力」大致相等**，而这个常数由延迟预算钉死。
...
1. 一个请求串行调用 5 个服务，每个服务 p99 都是 20 ms，端到端 p99 是 100 ms 吗？答不出 → 先读 [00-concepts §3 p50 / p90 / p99](../00-foundations/00-concepts.md)
2. 什么是近似最近邻（ANN）？它比精确最近邻快几个数量级，代价换的是什么？答不出 → 先读 [01-storage-engines §6 向量索引](../01-building-blocks/01-storage-engines.md)
3. 一个 key 被 30% 的请求同时读会发生什么？为什么加分片不解决这个问题？答不出 → 先读 [02-caching §4 热点 Key](../01-building-blocks/02-caching.md)
...
| Little's Law、分位数不可相加 | §2.2、§2.4 推容量与预算 | [00-concepts §2](../00-foundations/00-concepts.md)、[§3](../00-foundations/00-concepts.md) |
...
| **20–26 min** | 深挖一：**漏斗配比的守恒律**（候选数 × 单条算力 ≈ 常数） | **本题唯一的必答点。**说不出为什么不是两级、不是五级，前面白讲 |
...
**如果只剩 10 分钟**：跳过召回选型（一句"双塔 + HNSW，多路配额合并"带过），把全部时间给 §4.1 和 §4.5。
...
**先说为什么必须逐项摊开**：分位数不能相加（[00-concepts §3](../00-foundations/00-concepts.md)），但**预算可以相加**。预算表是一份契约：每一级拿到多少毫秒，超了就自己降级，不许去借下一级的。没有这张表，线上必然出现"某一级偷偷占用了别人的预算，直到某天它抖动一下，全链路一起超时"。
...
> **面试金句**
> "漏斗不是'为了省钱把模型变小'，它是一条守恒律：每一级的候选数乘以单条算力大致是个常数，这个常数由延迟预算钉死。所以真正的设计动作只有一个 —— 决定把它花在哪一级。多数团队本能地把它花在精排上，但把预算摊开你会发现，差不多一半的时间根本没花在算模型上，花在搬数据上。这就是为什么这题的第一个架构决策是特征放哪，不是模型选哪个。"
> "The funnel isn't about shrinking models to save money — it's a conservation law. At every stage, candidate count times per-candidate compute is roughly constant, and that constant is pinned down by your latency budget. So there's really only one design decision: which stage you spend it on. Most teams instinctively spend it on the final ranker. But lay the budget out line by line and you'll see roughly half the time isn't spent on models at all — it's spent moving data. That's why the first architectural call here is where features live, not which model you pick."
...
**推论 2**：**但这份数据小到荒谬。** 1,000 万物料 × 500 B = **5 GB**，加上 HNSW 向量索引（1,000 万 × 64 维 FP16 = 1.28 GB + 图结构约 2.6 GB）合计约 **9 GB** —— **每台排序服务器都放一份全量，绰绰有余**。跨网络取数的必要性从一开始就不存在。
**推论 3**：user 侧只有 1 个实体，20,000 key/s，**2 个 Redis 分片足够**（含副本 4 个）。同样是"在线特征存储"，两侧的容量需求差三个数量级。
...
```
 客户端 ─► API GW ─► Ranking Svc ×32   每台常驻：item 特征 5 GB + HNSW 索引 3.9 GB
                          │
    ┌─────────────────────┼──────────────────────┐
    ▼                     ▼                      ▼
...
**守恒律看出来了**：候选数逐级 ÷8 ~ ÷10，单条算力逐级 ×4 ~ ×10，**每一级的耗时都落在 5–25 ms 这同一个量级**。这不是巧合 —— 是延迟预算把它们钉在一起的。
...
1. **不重试。** 重试会在系统已经过载时把负载放大 2–3 倍（[`03-resilience-patterns.md`](../05-reliability/03-resilience-patterns.md) §3）；过载时正确的做法是准入控制而不是排队，见 [`../08-ml-systems/04-online-inference.md`](../08-ml-systems/04-online-inference.md)。唯一允许的"重试"是**对冲请求（hedged request：p95 之后再发一个副本，谁先回用谁）**，只在 GPU 精排这一跳做，且必须计入 GPU 容量。
...
**问题**：10 路召回各返回一批候选，分数的量纲完全不同 —— ANN 路给余弦相似度 [0,1]，协同过滤路给共现次数，热门路给曝光量，运营池给人工权重。**把它们归一化后放在一起排序是错的**（归一化只是把不可比藏起来）。正确做法是**配额（quota）+ 粗排统一打分**：每路按配额截断，合并后一律由粗排重新打一次分，召回分数只在**路内**用于截断。
...
| **内容指纹（simhash 64 位）** | 内容哈希 | **转载、搬运、同一视频不同上传者** —— 用户视角这就是重复 | 每条物料一个 64 位值，海明距离 ≤3 判重，指纹比对约 0.3 µs/对 |
...
| **HNSW**（efSearch=64） | 1.28 GB 向量 + 约 2.6 GB 图（M=32） | **8–12 ms** | ≈ 0.92 | 增量删除只能打墓碑，删除比例 > 20% 后 recall 显著下降，必须重建 |
| HNSW（efSearch=128） | 同上 | 16–24 ms | ≈ 0.96 | 直接超出 12 ms 预算 |
| IVF-PQ（量化） | 约 0.2 GB | 4–6 ms | ≈ 0.85 | 精度损失在长尾物料上更严重，且 nprobe 调参敏感 |

**选 HNSW + efSearch=64**：预算内 recall 0.92，而且**召回阶段的 recall 损失比你想的便宜** —— 漏掉的 8% 里大部分本来也进不了精排 top-100。真正致命的是下面这个：

**embedding 版本必须成对切换。** 用新模型算出的 user 向量去查用旧模型建的 item 索引，结果是**几何上有意义但语义上是随机的** —— 不报错、不超时、不触发任何服务级告警，只是推荐质量静默塌掉。这是这一节唯一会写进故障复盘的东西。

三条防线：①**向量带版本号**，检索时校验 `user_vec.model_version == index.model_version`，不匹配走降级路而不是硬查；②**双索引并存 + 原子切换**（新索引建好、影子比对通过后一次指针切换，旧索引留一版用于回滚，内存代价 3.9 GB → 7.8 GB，仍然装得下）；③**检测信号是召回路 top-1 相似度分布的突变** —— 正常时它稳定，版本错配时整体下移且方差变大，比等 CTR 掉下来快几小时。索引重建与增量的完整取舍（墓碑、over-fetch、embedding 升级的成本账）见 [`03-multi-tenant-vector-search.md`](03-multi-tenant-vector-search.md) §6。

**新物料 15 分钟可推**怎么做到：不靠重建索引，靠**新品直通召回路** —— 新物料入库后先进一个按时间排序的小索引（几万条，暴力扫描 < 1 ms）并带强制曝光配额，等下一次增量插入再写进主 HNSW 图。两条路并存，新品既不用等重建，也不会因为统计特征全是 0 而被粗排一刀砍掉。
...
|---|---|---|---|---|
| A. 全部走在线 KV | 500 KB | 2,000 万 key/s | +25–60 ms | 分片数 > 100 就该停手；单实体 key 热点有硬上限（DynamoDB 单 key **3,000 QPS**，Tecton 文档口径） |
...
**选 C。** 更新通路是近线推送：流式聚合作业每分钟把变化的 item 特征增量推给所有排序服务器（20,000 条变化/分钟 × 500 B = 10 MB/分钟，可忽略）。**代价说清楚**：①每台机器都要 9 GB 常驻内存，扩容时新实例要先加载完才能进负载均衡（约 40–90 s，必须做成就绪探针的一部分，否则第一批用户吃到全空的特征）；②特征更新不是强一致，各台机器之间可能差几十秒 —— 对统计类特征无所谓，对"下架"这类特征必须走另一条强一致通道（下架走过滤层的黑名单 bitmap，不走特征）。

**三条通用规则，与物料规模无关**（在线特征存储本身怎么建、时间点正确性怎么保，见 [`19-feature-store.md`](19-feature-store.md)）：

1. **延迟由 feature view 个数决定，不由特征个数决定。** 10 个特征分散在 5 个 view 里 = **5 次串行往返**（[Feast 文档](https://docs.feast.dev/how-to-guides/online-server-performance-tuning)，持续更新）。唯一的例外是 Redis：同一实体的所有 feature view 可以共用一个 hash key，往返恒为 1 次。**所以"把 user 侧的 4 个 view 合并成 1 个 hash key"是一次 12 ms → 4 ms 的改动，而且不需要动任何模型。**
...
3. **给单请求设字节上限和后端超时**，参考值 ≤ **2 MB** / **2 s**（Tecton 的 SLO 资格线）。本文 item 侧 500 KB 已经用掉四分之一 —— 这是推论 1 的另一个角度。
...
1. **服务端特征日志（feature logging）—— 唯一能把偏斜归零而不是"检测"的方案。** 线上把**实际喂给模型的那个特征向量** + 预测结果 + request_id 一起落盘，下一轮训练直接用这份日志 join 标签，不做离线回溯拼接。代价是日志量：1,000 条候选 × 500 B 太大，只记进入精排的 100 条 → 20,000 QPS × 100 × 500 B = 1 GB/s，采样到 10% 后 100 MB/s，可接受。
...
4. **别把 skew 当 drift 做**（成因分类与检测闭环见 [`../08-ml-systems/06-feature-and-data.md`](../08-ml-systems/06-feature-and-data.md)）：偏斜（skew）是拿线上特征分布**对训练集**比，抓的是"部署第一天就存在"的管道实现差异；漂移（drift）是**对上一个时间窗**比，抓的是随时间的变化。只做后者会永远漏掉前者。
...
③**降级产生的曝光必须打标，并且从训练样本中剔除或降权。** 这是全章最容易漏、后果最长尾的一条。开头那个事故就是它：兜底期的 3,400 万条曝光进了训练集，模型学会"热门就是好"，长尾曝光占比从 31% 掉到 19%。这是**退化反馈回路**（degenerate feedback loop：模型只推热门 → 用户只能消费热门 → 训练数据更热门 → 长尾坍塌）的标准起手式。可观测指标：**长尾覆盖率、平均热度排名、曝光基尼系数**。
...
|---|---|---|---|
| **全随机桶**（一小撮用户完全随机排序） | 桶内 CTR 掉 40%+，全局影响 = 桶大小 × 40% | **唯一能做无偏离线评估和倾向性加权（IPW：按一条样本被曝光的概率取倒数来加权，抵消"模型爱推什么就只能观测到什么"）的数据源** | 你想做任何离线评估可信度校准的时候 |
...
| **降级曝光污染训练集** | 两周后模型学会"热门就是好"，长尾坍塌 | 长尾覆盖率、平均热度排名、曝光基尼系数 | 曝光日志带降级标记；训练时剔除或降权；探索流量补采 |
| 全站爆款 item 成为热 key | 若 item 特征在远端 KV，单 key QPS 打爆分片 | 单 key QPS（DynamoDB 硬上限 3,000/key） | 本文方案天然免疫（进程内驻留）；方案 B 需要本地缓存 + 多副本（[`02-caching.md`](../01-building-blocks/02-caching.md) §4） |
| 新模型分数分布漂移 | 排序相对关系没变但绝对分数变了 —— 下游用绝对分数做阈值的模块（广告出价、曝光过滤）全错 | **score distribution / 校准度 / 预测熵**，三个都要监控 | 影子流量比对分布；分数下游一律用分位数而不是绝对值 |
| 排序服务扩容实例未加载完就进 LB | 新实例返回空推荐，占比 = 新实例/总实例 | 就绪探针通过率、单实例 candidate_count | **加载 9 GB 特征 + 索引（40–90 s）必须做成就绪探针的一部分**，不完成不进 LB |
...
v2  物料 1,000 万，QPS 20,000 ← 本文
    四级漏斗；双塔 ANN（HNSW 进程内）；GPU 精排；**item 特征全量进程内驻留**；
    分层降级 + 兜底列表；特征日志作训练样本源；探索流量作为常设基础设施。
...
|---|---|---|
| 「用一个大模型直接给全部物料打分，再取 top 10」 | 1,000 万 × 1 ms ≈ 2.8 小时/请求。这个答案暴露的是完全没有延迟预算概念 | "漏斗的本质是一条守恒律：每一级的候选数 × 单条算力大致是个常数。逐级把候选降 10 倍、把单条算力提 10 倍，四级下来每级都只花 5–25 ms。" |
| 「特征全放 Redis，请求时取出来拼成向量」 | 1,000:1 读放大 → 2,000 万 key/s、1,100 个分片、80 Gbps 内网流量。而且他通常还会说"延迟取决于特征个数" | "**item 侧必须本地化** —— 一千万物料的特征只有 5 GB，从一开始就不该跨网络。user 侧一次往返就够。另外延迟由 **feature view 个数**决定，不由特征个数决定。" |
...
| 「超时了就重试，或者等它返回」 | 排序请求的价值随延迟归零；重试在已经过载时把负载放大 2–3 倍 | "**超时即放弃 + 分层降级**：精排超时返回粗排 top 10（粗排结果一直留在内存里），特征超时用陈旧特征，全挂返回离线兜底列表。**而且降级产生的曝光必须打标并从训练集剔除**。" |
...
| 分位数不可相加、Little's Law、容量估算 | [00-concepts §2、§3](../00-foundations/00-concepts.md) ｜ [`../00-foundations/02-capacity-estimation.md`](../00-foundations/02-capacity-estimation.md) §2、§3 |
...
1. **漏斗的守恒律是什么？** 四级各自的候选数、单条算力、耗时是多少？（候选数 × 单条算力 ≈ 常数；10 万×0.25 µs=25 ms / 8,000×1 µs=8 ms / 1,000×10 µs=10 ms / 100×50 µs=5 ms）
2. **每请求的特征读放大是多少？** 这个数字推出的架构结论是什么？（1,000:1 → 走 KV 要 2,000 万 key/s 和 1,100 分片 → item 侧必须进程内驻留，而它只有 5 GB）
3. **在线特征取数的延迟由什么决定？** （feature view 个数，不是特征个数 —— 每个 view 一次往返；Redis 可以把同实体的多个 view 合成一个 hash key）
...
**下一篇** → [21-ab-experiment-platform.md](21-ab-experiment-platform.md)：分层分流、曝光口径与序贯检验 —— 为什么 82% 的方向一致率还不够用来做判决。
