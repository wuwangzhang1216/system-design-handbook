...
| 分位数与尾延迟、灰度发布 | §2.4 延迟预算；§4.4 一台 → 10% → 全量 | [00-concepts §3](../00-foundations/00-concepts.md#3-为什么平均值是骗人的p50--p90--p99)、[`05-release-engineering.md`](../03-saas-platform/05-release-engineering.md) |
...
4 亿 ÷ 86,400 = 4,630 QPS 均值 × 3（日内峰均比） ≈ 1.4 万 QPS 峰值
```
...
**这个数字意味着**：在这组假设下，4 GB **单机装得下**。因此起点应是**先全量复制、暂不分片**；副本主要服务于可用性、QPS 和就近访问，而不是容量。若真实词库、语言数量或模型特征让镜像超过单机安全水位，再进入 §4.2 的分片讨论。容量估算改变时，结论也要随之改变。
...
```
══════ 写路径（离线，与在线服务零共享）══════
 搜索提交事件 + 建议采纳事件 ─▶ Kafka ─▶ 流聚合（5 min 滚动窗口，watermark delay Δ_ooo=2 min；allowed lateness 另配）
   （绝不统计 typeahead 前缀本身）           │
...
> **图里的三个词**：**watermark（水位线）** = 对“事件时间大体推进到哪里”的估计，流聚合用它决定何时首次产出窗口结果；图里的 2 min 是生成 watermark 的最大乱序估计，不是“迟到事件还能收多久”。后者由独立的 **allowed lateness + state retention** 决定：保留状态期间可以修正，状态清理后只能走侧输出或回填（[`03-messaging-and-streams.md`](../01-building-blocks/03-messaging-and-streams.md) §6）｜**CMS（Count-Min Sketch）** = 用固定几 MB 内存估计海量 key 频次的概率结构，只会高估不会低估｜**heavy hitters** = 频次最高的那少数几个 key（§4.3 讲怎么把两者组合成实时热点层）。
...
**K 取多少**：返回 10 条，但节点存 **top-15**（每节点 120 B）。多存的 5 条是给返回期敏感词过滤和去重兜底的 —— 否则过滤掉 3 条后你只能返回 7 条，UI 会出现"越打字候选越少"的诡异体验。代入方案 C：结构本身约 `1,500 万 × 60 B = 0.9 GB`，top-15 约 `300 万 × 120 B = 0.36 GB`，字符串表约 0.24 GB，合计约 **1.5 GB 原始布局**；再乘运行时对象、对齐等 1.5 倍余量，得到约 **2.3 GB 的单镜像规划值**。双缓冲约 4.6 GB，再给构建临时内存和进程本身留余量，仍按 16 GB 机器规划。这里的数字是容量假设，不是跨语言、跨实现都成立的基准测试结果。
...
|---|---|---|---|---|
| A. **全量复制**（每节点一份完整 Trie） | 完整镜像 | 好（可无状态轮询） | 1 | **镜像能安全装入单机时的默认起点** |
...
**选影子构建**。它最直接的代价是接近 2× 的镜像内存，此外还有构建临时空间和短暂保留旧版本的峰值 —— 2.3 GB 单镜像通常要按至少 5 GB、实践中 6–8 GB 的进程水位规划；这里用 16 GB 机器留出操作余量。具体价格取决于云厂商、地区与采购方式，不把它写死成一个通用数字。

```
 ┌───────────── Typeahead 进程内存布局 ─────────────┐
 │  activePtr ─────▶ [ 镜像 v103, 只读, 2.3 GB ]    │ ← 1.4 万 QPS 全部读这里
 │    请求开始时做一次 acquire load 并登记读者 epoch │   热路径无互斥锁，但不是“零原子操作”
 │  shadowPtr ─────▶ [ 镜像 v104, 构建 / 预热中 ]   │ ← 后台线程独占写
 └──────────────────────────────────────────────────┘
 切换：atomic exchange(activePtr, shadowPtr)         ← 发布前先完成校验与预热
 回收：等旧 epoch 的读者全部退出后再释放；若采用固定 60 s grace period，
       必须同时强制请求硬超时 < 60 s，不能只靠“通常请求很快”的假设
```

**更好的形态是把构建移出服务进程**：Builder 集群产出一个不可变文件，服务节点先 `mmap` 新文件、校验并预热，再原子发布新的映射句柄；symlink 的原子 `rename` 只负责标记“最新版本”，**不会自动改变进程里已有的 mmap**。好处有三：构建只做一次而不是 12 次；页缓存由 OS 管理，进程重启后索引更容易变热；服务进程本身不再需要构建期的临时堆。
...
**案例顺读下一篇** → [17-object-storage.md](17-object-storage.md)
