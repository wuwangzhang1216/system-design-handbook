...
收件箱（inbox）只存 (post_id 8 B, author_id 8 B, score 8 B) = 24 B/条
  ...
  ⚠️ 这 960 GB **有一个前提**：ZSET 必须始终保持在 listpack 编码里（24 B/member）。
     一旦越过 zset-max-listpack-entries 切成 skiplist，就是 80 B/member = 3.2 TB，**3.3×**。
     而 Redis 的这个转换是**单向的，长度降回来也不会转回去** —— 所以它不是"平均多用点内存"，
     是"一旦发生就永久性地多花 3.3 倍"。怎么保证不越界见 §4.2 的裁剪一段（**本题最贵的一个坑**）
              ...
              └─ 同步写自己的 inbox（read-your-writes 特例，不进队列）
                     ...
                     按扇出量（活跃粉丝数）分优先级队列：       不扩散。只维护
                          ...
                          <300k → bulk     （p95 30 s）
                                      │
                        只写"近 7 天活跃粉丝"（约 20%）
                        ZADD inbox:{fid} + 同一 Lua 里 ZREMRANGEBYRANK 裁到 200
...
> ⚠️ **第 2 行的 $14k 是有前提的**：收件箱长度必须**始终**不越过 `zset-max-listpack-entries`。
> 这个前提靠下面「裁剪怎么做」那一段保证，**不是自动成立的** —— 用错裁剪策略，这一行会悄悄变成 3.2 TB / $47k。
...
**而且这个跃迁是不可逆的**：Redis 只做 `listpack → skiplist`，**长度掉回阈值以下也不会转回来**。
所以它不是一次"偶尔多用点内存"，是一次**永久性的 3.3× 涨价**。设计裁剪策略时必须给的是**硬上界**，不是期望值 —— 见下面。
...
**裁剪（trimming）怎么做 —— 这一段决定上面那张表成不成立。**

一个很自然的想法是**概率裁剪**：不要每次 ZADD 都 `ZREMRANGEBYRANK`，改成以 1/50 概率触发一次裁到 200 条，把裁剪成本摊薄到 2%。
**这个方案在这里是错的**，而且错得很隐蔽 —— 它是本题最贵的一个坑：

```
❌ 错误推理：「1/50 概率 ⇒ 平均每 50 次插入裁一次 ⇒ 最坏长度 200 + 50 = 250 < 256，仍在 listpack 内」

真相：两次裁剪之间的插入次数服从几何分布，均值 50，但没有上界。
      从 200 涨到越过 256 需要连续 57 次不触发：
      P = (49/50)^56 ≈ 32%
      ⇒ 约 1/3 的**裁剪周期**会越界

而且因为转换不可逆，这个概率是**沿时间累积**的，不是稳定在 1/3：
      P(某个收件箱曾越界) = 1 − (1 − 0.32)^(插入次数 / 50)
      250 次插入 → 86%     500 次 → 98%     1000 次 → 99.96%
      ⇒ 一个每天进十几条的收件箱，几周内就必然漂移过去
```

而 Redis 的 `listpack → skiplist` 转换是**单向的**：长度掉回 200 也**不会**转回 listpack。
于是每个收件箱只要"倒霉"过一次就永久停在 80 B/member，随时间推移**几乎全部**收件箱都会漂移过去：

| | 每 member | 2 亿用户 | 月成本 |
|---|---|---|---|
| 一直保持 listpack | 24 B | 960 GB | **$14k** |
| 漂移成 skiplist（概率裁剪的稳态） | 80 B | **3.2 TB** | **~$47k** |

**这就是 §2 那条"3.3×"警告的来源** —— 不是估算方法的问题，是裁剪策略的问题。
...
2. ~~保留概率裁剪，把阈值抬到 512~~ —— **这条只是把时间买长，不是解法，别选它。**
   ...
   `P(曾越界) = 1 − (1−0.0018)^730 ≈ **74%**`。**几何分布没有上界，抬阈值抬不出上界来。**
   而且 `zset-max-listpack-entries` 是**实例级**配置，抬到 512 会改变该实例上所有 zset 的编码 —— 只有收件箱独占实例时才安全。
3. **接受 skiplist，把 80 B/member 写进预算**。如果你本来就要存 800 条，这是唯一诚实的选项 —— 那就直接按 $180k 那一行做决策。

> 💡 **这个坑的通用形态**：任何"摊薄成本的概率触发"都只给你**均值**保证，不给你**上界**保证。
> 一旦下游有一个**硬阈值 + 不可逆的状态跃迁**（这里是 listpack→skiplist，别处可能是分片扩容、编码切换、降级开关），
> 概率方案就必须换成确定性方案，或者把阈值抬到概率上界之外。**面试里能主动说出这一条，比说出 24 B 值钱得多。**
...
| 收件箱未裁剪，单 key 涨到几十万成员 | 编码退化成 skiplist，内存爆炸 + `ZREVRANGE` 变慢 | 收件箱长度 p99.9；Redis `used_memory` 斜率变陡 | **确定性裁剪**（ZADD 与 ZREMRANGEBYRANK 放同一个 Lua）+ 单 key 长度告警（> 500 即告警，不等 OOM）。**别用概率裁剪** —— 见 §4.2 |
