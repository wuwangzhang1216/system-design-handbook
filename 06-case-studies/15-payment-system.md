...
**如果你是直接翻到这道题的**：这题的全部难度压在"超时到底意味着什么"上。答不出下面第 3 题，正文的胜负手你会看成一句正确的废话。
...
> **在一个你无法回滚的外部世界里，把"状态未知"收敛成确定结果。**
> 支付系统里最贵的一个词是 timeout：它的语义是"不知道"，不是"失败"。整篇正文都可以读成对这一句的展开 —— 三重保障是收敛手段，对账是最后的收敛手段，账本是收敛结果的存放处。
...
③ 在途并发（Little's law：在途 = TPS × 依赖延迟）
       ...
       ⇒ 必须异步/协程/虚拟线程，且每渠道设并发上限（bulkhead 隔板：给每个依赖一个独立的
          资源池，一个渠道被打满时其他渠道的请求仍有资源可用 —— 见 05/03 §5）
          max_inflight = 2,000
...
> **图里的四个词**：**fail-open** = 依赖不可用时**放行**而不是拒绝（反面是 fail-close；判据见 §4.6）｜**outbox** = 业务写和"待发消息"写进同一个数据库事务，再由投递器异步发出，避免"库写了消息没发"（[`03-messaging-and-streams.md`](../01-building-blocks/03-messaging-and-streams.md) §4）｜**T+1 清算文件** = 交易日次日渠道给出的权威流水，是最终的事实来源｜**append-only 复式分录** = 每笔钱同时记一"借"一"贷"、两边相等，且只追加不修改（§4.3）。
...
- **不要逐行查库。** 1,000 万行 × 3 渠道逐行点查 = 几小时。两边按渠道流水号排序做外排序 merge join（把两份数据各自排好序后并排扫一遍，用两个游标一次比完，内存里只留当前行），或都灌进列存做 full outer join —— 3 GB 在单机 DuckDB / Spark 上是分钟级。
...
| **短款** 我方有、channel 无 | 我方 `succeeded`，渠道文件里没有 | 我方误把 UNKNOWN 判成成功；渠道文件延迟 | 等一个文件周期；仍无则**冲正（reversal：新增一笔金额相同、方向相反的分录把它抵消，原分录一个字不改）**并告警 | < 0.001% |
