...
|---|---|---|---|---|
| 单租户边际成本/月 | **$0.01–1** | $1–20 | $50–500 | **$500–5,000** |
| 单集群/实例租户上限 | 10⁶+ | **500–2,000**（见下） | 100–500 | 1 |
| 隔离强度 | 应用/RLS 逻辑隔离 | 逻辑隔离 + 独立对象 | 进程与存储隔离 | 网络+计算+存储全隔离 |
| 单租户备份/恢复 | ❌ 极难（要恢复到影子库再回填） | ⚠️ 可 `pg_dump -n` | ✅ 原生 | ✅ 原生 |
| 单租户硬删除 | ⚠️ 大表 DELETE + vacuum | ✅ `DROP SCHEMA CASCADE` | ✅ `DROP DATABASE` | ✅ 销毁栈 |
| 单租户 PITR | ❌ | ❌ | ✅ | ✅ |
| 爆炸半径 | 全部租户 | 全部租户（共享实例资源） | 该实例上的租户 | 1 个租户 |
| per-tenant schema 定制 | ❌ | ✅（也是诅咒，见下） | ✅ | ✅ |
| 版本/发布节奏 | 全体同步 | 全体同步 | 可分批 | **可完全独立** |
| 合规适配（数据驻留/BYOK） | 难 | 难 | 中 | ✅ 天然满足 |
...
**它几乎总是正确的分片键**，因为绝大多数查询天然带 `tenant_id`，不会产生跨分片扇出。

代价：**分片内数据倾斜完全由租户大小决定**，你控制不了。所以分片键要预留一层间接：
...
**虚拟桶数取 1024–8192**，一次定死。桶数是唯一不能在线改的参数。

⚠️ **超大租户必须能突破单桶**：给 whale 租户启用二级键 `(tenant_id, sub_key)`，`sub_key` 取业务上天然的第二维度（project_id / workspace_id / 日期）。**这个逃生舱必须在第一天就在 schema 里留好位置**（哪怕暂时恒为 `'0'`），事后加等于全量迁移。
...
⚠️ **反模式：控制面进入数据面关键路径。** 每次请求去查控制面的路由表，等于把控制面的可用性乘进产品可用性。路由表必须能被数据面完整缓存并在控制面不可用时继续服务。
...
Cell 内部的组成、协调循环、跨 cell 迁移与 cell 级金丝雀见 [`03-saas-platform/01-control-plane.md`](../03-saas-platform/01-control-plane.md)；本节只讲**租户落到哪个 cell** 这一层。
...
2. **快照拷贝**：源库一致性快照（Postgres `pg_export_snapshot` / 逻辑复制槽）批量导入目标库。**限速**：不超过源库 IOPS 的 30%，否则会拖垮同 pool 的其他租户 —— 迁移本身就是最大的噪音邻居。
3. **增量追平**：CDC 把快照点之后的变更持续应用到目标。等到 **replica lag < 1 s** 才进下一步。
4. **冻结写入**：把该租户的写请求排队（不是拒绝），返回 `Retry-After`，或前端显示"同步中"。**冻结窗口 = 剩余 lag + 校验时间 + 路由生效时间**，目标 < 30 s。
5. **最终校验**：行数 + 关键表的 checksum（`sum(hashtext(row::text))`）比对。**不一致就回滚**，不要"先切了再说"。
6. **切换路由**：控制面更新 placement + 版本号，广播失效。DNS 型 router 这里要等 TTL。
7. **解冻**：排队的写请求放行到新位置。
8. **双读校验期**（1–7 天）：读走新库，同时异步采样读旧库比对，差异率上报。这是唯一能发现"漏迁了某张表"的手段。
9. **清理源数据**：确认无回滚需求后删除。**这一步经常被跳过**，结果半年后有人从旧库读到了幽灵数据。

**必须提前准备的两个东西：**
- **回滚脚本**（把 placement 改回去 + 反向 CDC），且演练过。
- **迁移期间的写入语义**：租户是否接受 30 s 只读？如果不接受，就需要双写 + 冲突解决，复杂度翻三倍。**先去问产品，再写代码。**

---

## 5. 噪音邻居：先量化，再治理

### 没有 per-tenant 计量就没有治理
...
| 噪音源 | 检测信号 | 治理手段 |
|---|---|---|
| 某租户突发流量 | 单租户 QPS 占比 > 25% | **令牌桶限流**（per-tenant），突发容量 = 稳态速率 × 10 |
| 某租户慢查询 | 单租户 DB 时间占比 > 30% | `statement_timeout` per-tenant（按 tier 设 1 s / 5 s / 30 s） |
| 某租户占满连接池 | 池等待时间 p99 上升 | **隔板**：把池按 tier 切分，任一 tenant 上限 = 池容量 × 20% |
| 大批量导入 | 写 QPS 尖峰 | 批量作业走独立队列 + 独立连接池，与在线路径物理隔离 |
| 长任务饿死短任务 | p99/p50 比值 > 20 | **公平队列**（下） |
| 存储无限增长 | 单租户存储超配额 | 软限额告警 → 硬限额拒写（保留读） |
...
**这里有一个 AI 场景特有的坑**：LLM 请求的 `cost` 在**执行前不可知**（输出长度未知）。做法是"先按 `max_tokens` 预扣，完成后按实际用量退还差额"—— 与第 2 篇的 TCC 预留模式同构。不做预扣的话，一个开着 `max_tokens=32000` 的租户能把整个队列拖死。

---

## 6. Postgres 行级安全（RLS）：正确用法与两个坑
...
| **非 LEAKPROOF 函数阻止谓词下推** | 规划器不允许把用户提供的、可能泄露信息的函数推到 RLS 过滤之下 → 扫描量放大 | 避免在 WHERE 里用自定义函数；必要时 `ALTER FUNCTION ... LEAKPROOF`（需 superuser，且你要为正确性负责） |
| 策略里调用函数 | 每行调用一次 | 策略里只用 STABLE 函数（`current_setting` 是 STABLE，安全） |
| 分区剪枝失效 | 按 tenant hash 分区时，RLS 谓词形式不匹配分区键表达式 | 分区键与策略表达式写成同一形式；验证 `EXPLAIN` 里出现 `Partitions removed` |

**开销量级**：谓词能走索引时 **5–15%**；击穿索引时可以是 **10–100×**。所以上线 RLS 必须配一次全量 `EXPLAIN (ANALYZE, BUFFERS)` 回归。

> **面试金句**
> "RLS 我会用，但我不把它当**唯一**的隔离层。它是**纵深防御的最后一道**：应用层照样在每个 query 里显式带 `tenant_id`，RLS 是防止我漏写的兜底。理由很实际——RLS 只在数据库连接上生效，而我的系统里还有缓存、搜索索引、对象存储、向量库、消息队列，它们没有 RLS。**隔离必须在每一层各自实现一遍**，指望一个数据库特性覆盖全栈是错的。"
...
| **恢复单租户到某时间点** | **PITR 到影子实例 → 导出该租户 → 回填**（小时级） | 同左但导出快 | 原生 PITR，分钟级 |
| 硬删除 | 分批 `DELETE` + vacuum，大表需数小时 | `DROP SCHEMA CASCADE` 秒级 | `DROP DATABASE` |
| 导出（数据可携带权） | 按 tenant_id 全表扫，要限速 | 快 | 快 |
...
**必须提前定的两个数字**：单租户恢复的 RTO（现实值 2–6 小时，别承诺 15 分钟）和 RPO（= WAL 归档间隔）。

**删除的正确做法（呼应上一篇的 crypto-shredding）：**
...
**per-tenant KEK 是 L1 pool 下唯一实用的删除加速器**：租户数据按其 KEK 加密，删 KEK 让所有副本（包括备份里的）同时失效，剩下的物理删除可以慢慢做。代价见上一篇：KEK 存储绝不能开 PITR。
...
1. **分片内二级分裂**：启用 `(tenant_id, sub_key)`，把该租户的数据打散到同一分片的多个分区。最便宜，但只解决单表热点，不解决资源竞争。
2. **专用分片**：迁到只有它一个租户的分片（走第 4 节剧本）。解决资源竞争，保留同一套代码和发布节奏。**这是 90% 的 whale 的正确答案。**
...
### b) 前缀缓存的跨租户泄露（已被实证，不是理论风险）

这是本手册里最需要单独强调的一条：**KV / prefix cache 既是 2026 最大的性能杠杆，也是最大的跨租户泄露面。**

- [PROMPTPEEK（NDSS 2025）](https://www.ndss-symposium.org/wp-content/uploads/2025-1772-paper.pdf) 利用 KV cache 共享造成的**服务顺序/时序侧信道**逐 token 重建他人的 prompt。实测成功率：**已知 prompt 模板 99%、反推模板本身 98%、无任何背景知识 95%**；已知模板时约 **60 次请求**即可套出受害者的性别/年龄/体重/身高。攻击面覆盖 vLLM、SGLang、LightLLM、DeepSpeed；已向厂商披露，ByteDance 已确认该侧信道。
- 另一面：前缀缓存能把 TTFT 降低数倍、命中率在共享 system prompt 场景可达 70–90%、聚合吞吐 +30–50%（收益幅度存在争议，也有实测报告在特定负载下吞吐**下降** 36% 的相反结论 —— 取决于前缀重叠率与并发数）。
...
**用托管 API 时同样要处理**：OpenAI 的 ZDR（零数据保留）**不消除 prompt cache 驻留** —— ZDR 排除 abuse monitoring 中的客户内容并强制 `store=false`，但**仍会存储加密的 prompt cache tensors，最长 24 小时**。这一点必须写进 DPIA 与数据流图，不能因为"我们开了 ZDR"就跳过。

### c) 微调 / 适配器隔离

多 LoRA 共享底座是主流成本方案（一个 base model + N 个租户适配器），但 **vLLM 本身不提供租户隔离**：一个 Pod 内的 batch 共享同一个 KV cache 与 scheduler。
...
⚠️ 代价：**冷启动次数从 O(并发) 变成 O(租户 × 并发)。** 隔离是免费的，冷启动不是。公告未披露最大租户数、冷启动放大倍数与定价细节 —— **三项都必须自测**。对策仍然是 bridge：小租户走 pool 模式，大租户/高合规租户走 silo 模式。
...
3. **事后改分区键。** 等于全量数据迁移。若业务天然存在跨租户协作（共享工作区、跨组织审批），**要么把协作对象本身作为分区单元，要么承认该子系统不适用分片模型**。
