...
**最常见的错误是从 ⑥ 开始。** 分片是不可逆的复杂度承诺：跨分片事务没了、跨分片查询没了、二级索引（secondary index）变难、运维成本永久增加。
...
| **S1 读扩展** | 1k – 10k | + 只读副本（read replica） 2–3 个<br>+ Redis 缓存层<br>+ CDN | DB 读、N+1 查询 | 主库 CPU > 60%；p99 抖动；慢查询日志变长 | $3k – 15k |
| **S2 写路径瘦身** | 10k – 50k | + 写异步化（队列/Outbox）<br>+ 连接池代理（pgbouncer）<br>+ 表分区（partitioning） | **连接数**、写 IOPS、锁竞争（lock contention） | 连接数逼近 `max_connections`；WAL 生成速率打满；vacuum 追不上 | $15k – 60k |
...
| 4. CQRS / 物化读模型（materialized read model） | 复杂查询 10–1000× | 双写（dual write）复杂度、最终一致，见 [`02-architecture-patterns/02-event-driven-and-cqrs.md`](../02-architecture-patterns/02-event-driven-and-cqrs.md) |
| 5. 搜索/分析外置（ES/ClickHouse） | 聚合查询 100× | 一份数据两处存，需要 CDC |

⚠️ **只读副本的隐形陷阱**：用户改完资料立刻刷新，读到旧值。解法不是"关掉副本"，而是**按会话粘性（session stickiness）**：写入后的 N 秒内（或直到 LSN 追上）该用户的读走主库。在 cookie/会话里存一个 `write_lsn` 是最干净的做法。
...
| **一致性哈希（consistent hashing） + 虚拟节点（virtual node）** | ~`1/N` | 中 | 缓存层（Redis/Memcached）、无状态路由 |
...
| UUIDv4 | ❌ | ❌ | 128 bit | **B-Tree 索引杀手**（随机插入导致页分裂 page split），避免 |
