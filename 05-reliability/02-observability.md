...
|---|---|---|---|---|
| 数据形态 | 预聚合的数值时序 | 离散的结构化事件 | 因果关联的 span 树 | 栈采样的火焰图（flame graph） |
...
**最后一行是 Staff 与 Mid 的分水岭**：`成本 / 用量 / 计费` 这类需要精确、可审计、可按任意维度回溯的数据**属于数据仓库（data warehouse），不属于可观测性系统**。塞进 TSDB 你会同时得到高账单和不准的数字。
...
**USE 里最被忽略的是 Saturation。** 利用率 80% 和"利用率 80% + 队列长度 500"是完全不同的两个系统，后者已在雪崩边缘。**几乎所有"CPU 才 70% 为什么这么慢"的答案都在 Saturation 那一栏**（连接池（connection pool）排队、线程池（thread pool）队列、GC 停顿、磁盘 IO 深度）。

### 分位数（percentile）聚合的正确性陷阱（必考）
...
托管日志平台的摄入（ingestion）单价量级 **$0.5–3 / GB**（2026 年中量级，随时变动；存储与查询另计）。78 TB/月 × $1/GB ≈ **$78,000/月**。
...
| **人工/自动干预**：谁在什么时候改了什么 flag | 心跳（heartbeat） / 探针（probe） |
...
**它解决的是其他三支柱结构性看不见的问题：** 所有请求均匀变慢 20%（trace 显示每一跳都慢一点 = 零信息）；时间花在没有 instrument 的库里（那里没有 span）；内存缓慢增长到 OOM（只有 heap profile 能归因到分配点）；锁竞争（lock contention） / goroutine 泄漏（完全不可见）；发布后 CPU 涨 30%（metric 只说涨了，不说哪个函数）。
...
**杀手级用法是差分火焰图（differential flame graph）**：`profile(部署后) − profile(部署前)` 直接高亮新增的栈帧。一次"部署后 p99 涨了 200ms"的排查，差分火焰图通常 **5 分钟**定位到函数，trace + 日志要**半天**。**把它接进金丝雀流程**——金丝雀实例与基线实例自动做 profile diff，作为发布门禁（release gate）的一项。
...
**Collector 是关键，不是 SDK。** 真正的收益是拿到一个**厂商无关（vendor-neutral）的解耦点**：
...
**两年内至少 5 次破坏性改名。跨版本查询不做 `coalesce` 会静默丢数据（silent data loss）——不是报错，是曲线上出现一段莫名其妙的空白。**

### 四条必须写进设计文档的对策

**① 在采集与查询之间加一层内部规范化（normalization） schema**，把上游的动荡挡在自己的命名空间之外：
...
| 日志分层保留（tiered retention）（热 7d / 温 30d 降采样（downsampling） / 冷 13m 归档到对象存储（object storage）） | 40–70% 存储费 | 查冷数据慢（分钟级） |
...
**① 基于症状（symptom-based），不基于原因（cause-based）。**
...
**③ 告警疲劳（alert fatigue）是可以量化的。** 经验阈值：**一个班次（12 小时）超过 2 条 page 级告警，人就开始批量确认而不阅读。** 这不是纪律问题，是注意力的物理上限。一个可类比的公开数据点：在高频权限确认场景中观测到约 **93% 的自动批准率**——**当确认变成条件反射，它就不再是控制**。告警也一样。
...
- 子代理（sub-agent）的探索上下文（数万 token）**只回传 1,000–2,000 token 的摘要**。必须保留原始探索轨迹的引用路径，否则事后无法复审父代理为什么做了那个决定。
...
| 日志里没有 trace_id | 分布式排障退化成时间范围大海捞针 | trace_id 是结构化日志（structured logging）的必填字段 |
