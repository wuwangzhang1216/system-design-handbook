...
> 事故响应的第一目标通常是**安全地减少用户影响（mitigation）**，完整根因分析留到影响受控后；但为选择安全动作所需的最小诊断与证据保全可以并行进行。
> 而混沌工程（chaos engineering）的价值不在"把东西弄坏"，在于**逼你先写下你以为会发生什么**。
>
> **阅读分层与边界**：§1–§9 是事故响应 Core，§10–§12 是演练与 AI 扩展。严重级别、分钟数、完成率和法律时限均为示例/时间快照；团队流程、合同与现行法律优先。
...
周五晚上八点，支付成功率从 99.9% 掉到 70%。二十个人涌进同一个 Slack 频道，三个人同时在翻日志，
CTO 每五分钟问一次"还要多久"，有人没吭声就重启了一个服务，另一个人正准备回滚 —— 但不确定该回滚哪一次部署。
两小时后问题解决了。第二天你翻聊天记录才发现：故障发生 12 分钟时就有人说过"像是 20:03 那次配置变更"，
那句话被埋在 400 条消息里，没有人接。复盘会开得很好，写了 12 条行动项，三个月后完成了 2 条。

**需要先懂的概念**

| 概念 | 一句话 | 详见 |
|---|---|---|
| SLO / 错误预算 / 燃尽率 | 事故的严重程度可以换算成"烧掉了本月预算的百分之多少" | [01-slo-and-error-budget.md §5](01-slo-and-error-budget.md#5-错误预算与燃尽率burn-rate) |
| 多窗口多燃尽率告警 | 长短两个窗口同时超阈值才告警，这决定了你多久能发现故障 | [01-slo-and-error-budget.md §6](01-slo-and-error-budget.md#6-多窗口多燃尽率告警multi-window-multi-burn-rate-alerting) |
| 降级开关与降级矩阵 | "止血"能按的按钮，必须在故障之前就存在并演练过 | [03-resilience-patterns.md §7](03-resilience-patterns.md#7-优雅降级graceful-degradation必须提前定义降级到什么) |
| 级联故障与亚稳态失败 | 为什么原因消失了系统还起不来，为什么恢复要限流后逐步放量 | [03-resilience-patterns.md §1、§9](03-resilience-patterns.md#1-第一原理你控制不了故障只能控制传播) |
| 静态稳定性 | 故障时不依赖控制面；这决定了可用区演练能不能通过 | [03-resilience-patterns.md §10](03-resilience-patterns.md#10-静态稳定性static-stability不要在故障时依赖控制面) |
| 灰色故障 | 健康检查绿着、能力已经掉光的那种故障，最难被检测到 | [01-fundamentals §8](../00-foundations/01-fundamentals.md#8-失败模型你要防的到底是什么) |

**这一章要回答的问题**

1. 事故发生的第一分钟，谁来决定做什么？为什么这个人不该是最懂这个系统的那位？
2. 为什么"先定位根因"是错的？哪些止血动作根本不需要知道原因？
3. 复盘写了 12 条行动项、三个月后完成 2 条、同样的事故又发生了 —— 问题出在复盘的哪一步？
4. 混沌工程和"随机杀 Pod"差在哪？在哪几个条件满足之前，你根本不该开始做它？

**本章新引入的术语**

| 术语 | English | 一句话定义 |
|---|---|---|
| 事故指挥官 | Incident Commander (IC) | 事故期间唯一有决策权的人，负责分派和判断，**不亲自敲键盘调试** |
| 止血 | mitigation | 让用户影响停止的动作（回滚、切流、降级、扩容），不要求知道为什么坏 |
| 严重级别 | severity level | 按**用户影响**划分的事故等级，决定响应速度、升级路径和是否强制复盘 |
| 升级 | escalation | 把事故交给更高级别的响应者，或提高它的严重级别；升级早了永远不该被批评 |
| 无责复盘 | blameless postmortem | 认定人在当时的信息下做了合理的事，转而追问系统为什么允许这个合理选择造成事故 |
| 事后诸葛偏差 | hindsight bias | 已经知道答案之后回看，觉得当时那个信号"显然"应该被发现 |
| 因果链 / 瑞士奶酪模型 | causal chain / Swiss cheese model | 事故是多个各自无害的因素恰好对齐的结果，不存在单一根因 |
| MTTD / MTTA / MTTR | mean time to detect / acknowledge / recover | 故障发生到被检测到 / 告警到有人确认 / 检测到用户影响结束，各自的平均时长 |
| 混沌工程 | chaos engineering | 先定义稳态、写下假设，再在受控的范围内注入故障，用来验证那个假设 |
| 稳态 | steady state | 系统正常时**业务指标**所处的区间（"每分钟成功下单 800–1200"），实验拿它当判据 |
| 爆炸半径 | blast radius | 一次故障或一次实验最多能影响到的范围（一个实例 / 1% 流量 / 一个可用区） |
| RTO / RPO | recovery time / point objective | 多久必须恢复服务 / 最多允许丢掉多长时间的数据 |
| 轨迹 | trajectory | 一次 Agent 运行里全部步骤、工具调用与中间输出的完整记录 |
...
1. **优先安全缓解，完整 RCA 后置。** 回滚、切流、降级、卸载或扩容常不需要知道最终根因，但要先确认动作适用：schema/数据是否向后兼容、备用区是否有容量、降级是否安全。必要的窄诊断与证据保全可并行，不能为追求“先按按钮”制造第二次事故。
2. **决策权集中，工作分散。** 一个人（IC）决定做什么，其他人执行。没有 IC 的事故会出现"两个人同时回滚不同版本"。
3. **沟通是独立工种。** 让在修问题的人同时回答"什么时候好"是最有效的拖延手段。

> **面试金句**：
> “事故中我把工作分成两条并行线：IC 选择最可能降低影响、且风险已知的可逆动作；诊断人员只做足以支持下一步决策和证据保全的调查。已知无效或可能破坏数据兼容性的回滚不会做，‘排除变量’不值得拿用户和数据做实验。完整因果分析等影响稳定后再展开。”
...
**小团队的现实版本**：IC、Ops、Comms 可以兼任，但要明确当前帽子和唯一变更执行者。高严重事故最好尽快拆出指挥与操作角色；人数不足时先求清晰，不把“至少三人”当阻碍宣告事故的门槛。
...
**安全/隐私事件走专门分支**。例如 GDPR Art.33 在适用且需通知监管机构时，以知悉为起点要求尽可能在 72 小时内通知；是否触发、通知对象和分阶段内容依风险判断而定。立即拉法务/隐私负责人并保护证据，不从本手册的一句话自行作法律结论。
...
> 📖 **读图要点**：Sev 状态直接进入 `Mitigating`，表示无需等到“最终根因已知”才减少影响；不表示禁止并行做安全判断、证据保全与假设验证。`Monitoring --> Mitigating` 表示观察期反弹就回到缓解；`PostmortemDue` 则把高严重事故的复盘做成流程状态，而非事后自觉。
...
各段的压缩手段（按 ROI 排序）：
  MTTD  ← 基于 SLO 的多窗口燃尽率告警（见 05/01），而不是阈值告警
        ← 合成监控（synthetic monitoring：用机器人按固定脚本定期走一遍关键旅程，
          即使此刻没有真实用户，也能回答"现在能不能用"）补上黑盒视角
...
**关键判断**：
> 很多团队最大的可控杠杆在缓解段，但“90%”必须用自己的事故时间线验证。检验方法是列出每条关键旅程已演练的恢复动作、适用前提、完成时间和失败方式；没有可在 SLO 所需时间内执行的动作，才是明确缺口。“至少 3 个按钮”可用于启发，不是成熟度定律。
...
3. **状态页更新时间来自沟通政策/SLA**；例如 Sev-1 宣告后 10 分钟内首次更新。先确认影响再发布，不编造原因或 ETA。
...
**判据**：行动项必须可验证并改变未来条件。优先改系统、流程、工具和决策信息；培训/值班能力建设也可能合理，但不能写成“以后更仔细”。“增加 dry-run 并验证使用率/误操作率”比“加一个可能被习惯性点掉的确认弹窗”更可检验。
...
| **行动项完成率** | 到期完成 / 总数 | 示例：> 80% | 低完成率说明范围/优先级/owner 有问题；阈值按团队基线设 |
| **变更相关事故占比** | 由部署/配置变更引发 | 通常 60–75% | 这个数字高不是坏事，说明归因清晰；**用它来论证渐进式发布（progressive delivery：金丝雀、分批、按比例放量，让每次变更的影响面可控且可秒级回滚）的投入** |
...
| 运维工作占比 | **≤ 50%** 工作时间 | 超过说明系统在吃人；剩余 50% 必须用于消除运维负担（toil：手工的、重复的、可自动化的、随规模线性增长的那类活） |
...
| 3 | 生产 | **1% 流量 / 1 个 cell**（单元：整个技术栈复制出的一份，各自服务一个用户子集，见 [05-scaling-playbook.md](05-scaling-playbook.md) §7） | 阶段 2 连续通过 |
| 4 | 生产 | **1 个可用区** | 有静态稳定性容量 |
| 5 | 生产 | **1 个 region**（游戏日 game day：预先排期、多团队一起参加的大规模故障演练） | 高层批准 + 客户沟通 |
...
| 3 | **杀掉 1 个实例** | 流量 5 秒内重新分布，无错误 | 优雅关闭（graceful shutdown：收到终止信号后先从负载均衡摘掉自己、把在途请求处理完再退出）没做，在途请求被砍断 |
...
| 8 | 时钟跳变（clock skew） ±5 分钟 | 无影响 | 令牌校验全失败、租约（lease：一份带到期时间的授权，不主动续期就自动失效）误判过期 |
...
- 单元化架构（cell）能限制演练影响：若有 `K` 个 cell，单 cell 故障影响的是它实际承载的租户/流量权重；只有均匀分配且没有共享依赖时才近似 `1/K`。可选择低权重 cell 演练，并先核对控制面、身份和存储等共享依赖。见 [`03-saas-platform/01-control-plane.md`](../03-saas-platform/01-control-plane.md)。
...
传统系统的变更 = 代码 + 配置。Agent 系统的变更包括：**提示词（prompt）、工具定义、模型版本（供应商可能悄悄换权重）、检索索引、embedding 模型（把文本转成向量的模型，向量检索的地基）、rerank 模型（对初步检索结果重新排序的模型）、上下文管理策略、温度/推理预算参数**。
...
提示注入（prompt injection：攻击者把指令藏在模型会读到的内容里 —— 网页、文档、工具返回值 —— 诱导它执行本不该执行的动作）在 2026 年仍是**未解决的架构性问题**：自适应攻击对 12 个已发表防御多数达到 >90% 成功率。**这意味着"检测到注入"不是一个可靠的事故检测手段** —— 你的事故检测必须建在**动作侧**（哪些高影响动作被执行了、哪些数据被外发了），而不是输入侧。
...
|---|---|
| **事故期间执着于完整根因、迟迟不缓解** | 延长影响。先做支持安全缓解的最小诊断；完整因果分析后置，或交给另一组并行推进 |
...
混沌实验需要与风险匹配的准入：可观测稳态、明确 owner、有限爆炸半径、中止机制与恢复验证。生产实验还需要变更审批和客户/合规边界；行动项完成率 >70% 只能作为示例治理信号，不是做任何小型预发故障注入的硬门槛。

---

## 这一章的三句话

1. **事故中优先安全地减少影响，完整根因分析后置。** 回滚、切流和降级常可在根因未知时执行，但必须满足数据兼容、容量和安全前提；最小诊断与证据保全并行，已知无效的回滚不做。
2. **无责不是宽容，是一条方法论。** 把"谁做错了"换成"在当时那个人能看到的信息里，为什么这个判断是合理的" —— 只有后者能产出可执行的改动。判据很硬：每一条行动项都必须是对系统、流程或工具的改动，"以后要更仔细"不是行动项。
3. **混沌工程是受控实验，不是随机破坏。** 从预发/单实例的小爆炸半径开始，具备稳态、中止与恢复验证后再扩大；假设被证伪可以是收获，但实验长期不失败也可能说明假设正确，是否扩大要看风险和信息价值。
...
**目录下一篇** → [05-scaling-playbook.md](05-scaling-playbook.md)

**主学习链下一篇** → [微服务 vs 模块化单体](../02-architecture-patterns/01-microservices-vs-modular-monolith.md)（完成 `01 building blocks → 05 reliability` 后进入架构模式）。
