...
| 概念 | 一句话 | 详见 |
|---|---|---|
| SLO / 错误预算 / 燃尽率 | 事故的严重程度可以换算成"烧掉了本月预算的百分之多少" | [01-slo-and-error-budget.md §5](01-slo-and-error-budget.md) |
| 多窗口多燃尽率告警 | 长短两个窗口同时超阈值才告警，这决定了你多久能发现故障 | [01-slo-and-error-budget.md §6](01-slo-and-error-budget.md) |
| 降级开关与降级矩阵 | "止血"能按的按钮，必须在故障之前就存在并演练过 | [03-resilience-patterns.md §7](03-resilience-patterns.md) |
| 级联故障与亚稳态失败 | 为什么原因消失了系统还起不来，为什么恢复要限流后逐步放量 | [03-resilience-patterns.md §1、§9](03-resilience-patterns.md) |
| 静态稳定性 | 故障时不依赖控制面；这决定了可用区演练能不能通过 | [03-resilience-patterns.md §10](03-resilience-patterns.md) |
| 灰色故障 | 健康检查绿着、能力已经掉光的那种故障，最难被检测到 | [01-fundamentals §8](../00-foundations/01-fundamentals.md) |
...
1. **事故的第一目标是止血，不是找根因。** 回滚、切流、开降级开关这三个动作都不需要知道为什么坏，而 90% 的 MTTR 改善恰恰来自这一段 —— 团队常年在优化告警，真正的瓶颈却是"发现了却不知道该按哪个按钮"。你的系统有几个"5 分钟内能明显改善指标"的按钮，少于 3 个就是缓解能力不足。
2. **无责不是宽容，是一条方法论。** 把"谁做错了"换成"在当时那个人能看到的信息里，为什么这个判断是合理的" —— 只有后者能产出可执行的改动。判据很硬：每一条行动项都必须是对系统、流程或工具的改动，"以后要更仔细"不是行动项。
3. **混沌工程是成熟度的结果，不是原因。** 没有稳态定义、没有一键中止、行动项完成率不到 70% 的团队做故障注入，只是在给自己额外制造事故；而在真正做起来之后，**假设不成立才是收获** —— 实验从不失败，说明爆炸半径设得太保守，没有产生任何信息。
