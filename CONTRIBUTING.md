...
- ✅ `消费端必须幂等（同一条消息处理两次和处理一次的效果相同 —— 因为分布式系统里只有 at-least-once 投递）。完整实现见 [00/01 §5](00-foundations/01-fundamentals.md)。`
- ❌ `消费端必须幂等（见 [00/01 §5](00-foundations/01-fundamentals.md)）。`
