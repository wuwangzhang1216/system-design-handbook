...
| 概念 | 一句话 | 详见 |
|---|---|---|
| 强一致 vs 最终一致 | 写完后任何人立刻读到新值 vs 只承诺最终收敛、中间可能读到旧值 | [00-concepts §6](../00-foundations/00-concepts.md#6-什么是一致性--一个词两种完全不同的意思) |
| 乐观并发 / CAS | 不加锁，提交时用版本号检查这行有没有被别人改过 | [00-concepts §7](../00-foundations/00-concepts.md#7-事务与隔离级别) |
| 读己之写 | 只保证"同一个会话里，自己刚写的一定读得到"的弱一致档位 | [00/01 §4](../00-foundations/01-fundamentals.md#4-两条正交轴副本一致性与事务隔离) |
| 幂等键 | 客户端生成、服务端去重的请求标识，让重试不产生第二次副作用 | [00/01 §5](../00-foundations/01-fundamentals.md#5-幂等idempotency分布式系统的第一公民) |
| 上下文腐化与 distractor | 输入越长表现越差；单个不相关片段就能拉低准确率 | [02 §2](02-context-engineering-and-rag.md#2-上下文会腐化context-rot这一节的实证决定了后面所有设计) |
| 前缀缓存与失效级联 | 记忆放在提示词的哪一段，直接决定它每轮花多少钱 | [02 §9](02-context-engineering-and-rag.md#9-上下文组装顺序与前缀缓存的硬约束) |
...
|---|---|---|---|---|---|---|---|
| ① | **会话上下文**<br>conversation context | messages 数组、tool_use / tool_result、thinking 块 | 单会话，分钟–小时 | 10K–1M token（40KB–4MB） | 正常消息可 append；编辑、删除、分支与并发续写需要版本号或 CAS | 对象存储 append-only JSONL（一会话一对象）+ 元数据行在 Postgres；活跃会话在 Redis 做热副本 | 会话续不上，用户从头再来 |
| ② | **任务状态**<br>task / execution state | 当前 step、待办图、重试计数、已发生的副作用与幂等键（idempotency key）、预算余额 | 单任务，秒–天 | 通常 KB 级；上限由数据库行大小、写放大和恢复 SLO 倒推 | 涉及租约、预算和副作用时需强一致 + CAS/事务 | 关系库单行（Postgres `FOR UPDATE` 或 `version` 乐观锁 optimistic locking）/ DynamoDB 条件写；或交给 durable execution 引擎 | 恢复退化成重跑，**副作用重复执行**（重复扣款、重复发信） |
| ③ | **长期记忆**<br>long-term memory | 用户偏好、项目约定、踩过的坑、"上次我们决定用 pnpm" | 跨会话，月–年 | 每用户 KB–低 MB | 推荐内容可最终一致；用户刚修改/删除的偏好应支持 read-your-writes | Postgres 结构化事实表（可枚举、可审计、**可删除**）+ 可选向量索引作二级召回；代码类场景直接用文件系统 | 多数情况体验退化；权限、合规或明确约束被记错时也会影响正确性 |
...
> **判据（decision rule）**：先问它有没有权威真相源、允许多旧、读取成本多高，而不是套一个固定的 200ms 门槛。能按当前任务的延迟预算和可用性目标稳定回源，就保存引用或短期缓存；只有真相源里没有、且跨会话仍有价值的信息，才进入长期记忆。
...
- 推论：**多注入一条不相关的记忆通常是净负收益**，不是“反正也不贵”。记忆检索和 RAG 都要权衡精确率、召回率与漏掉关键信息的代价；记忆通常更偏重精确率，因为错误偏好会跨会话反复生效，但安全规则或明确约定仍不能因召回不足而漏掉。
...
以下数值是便于启动实验的示例，不是跨产品标准：

1. **访问衰减（access decay）**：可从 `score = confidence × exp(-λ · days_since_last_used)` 起步，例如 λ 取 1/90（半衰期约 62 天），再用真实复用率调参。低分先降到冷存储，不宜直接删除用户明确保存的内容。
2. **容量上限**：每个 scope 设可配置上限（例如每用户 500 条语义记忆）。超限时按价值和合规策略淘汰；用户可见或主动保存的记忆要告知，系统派生的低价值条目可按产品政策清理。
3. **复核候选**：例如 `confidence < 0.7`、`use_count == 0` 且存活 > 90 天时进入复核/清理队列。未被召回也可能是检索器漏召，不能据此断言“没有价值”。

### 用户可编辑与可删除（按适用法律和组织政策落实）

删除权（right to erasure，GDPR Art.17）落到 Agent 系统上，**要删的不止那一行记录**。上线前必须能回答"删干净了吗"的清单：

| 载体 | 是否含用户数据 | 删除方案 |
|---|---|---|
| 记忆表主记录 | 是 | 从主服务与检索路径移除；是否立即物理删除、加密擦除或先受限隔离，要按适用法律、合法保留义务和组织政策确定，不能只靠永久 soft-delete 标志 |
| supersede 链上的历史版本 | 是 | 同批删 |
| **向量索引里的 embedding**（把一段文字映射成的一串数字，见 [`02 §5.2`](02-context-engineering-and-rag.md#52-embedding-选型2026-年中量级随时变动)） | **是**（embedding 是个人数据的派生物） | 必须支持按 `memory_id` 定点删除 → **这直接排除掉不支持删除的索引结构** |
...
⚠ **架构后果**：要让删除请求可执行，记忆必须可枚举、可归因到数据主体，并能沿主库、索引、摘要、日志和备份的生命周期追踪。在线副本应在承诺期限内不可访问；备份可按既定保留期到期或通过加密擦除失效。具体例外与期限需由法务/隐私团队确认。**从第一天就用结构化条目存记忆，不要把它混进不可寻址的 system prompt 自由文本。**
...
Anthropic 的服务端 context editing（beta header `context-management-2025-06-27`，截至 2026-07 **仍是 beta**）给出了可抄的参数。**完整参数表与三个坑见 [`02-context-engineering-and-rag.md §10`](02-context-engineering-and-rag.md#10-压缩与摘要什么时候触发保留什么丢什么)**，这里只补记忆视角特有的两条：
...
**大小目标 < 64KB。** 超过就说明你在 checkpoint 里存内容而不是存状态，恢复会变慢、写放大（write amplification：一次逻辑上的"存一下"引发多少次实际磁盘写入；checkpoint 每步一写，单次越大总写入量涨得越凶，见 [`00-concepts §11`](../00-foundations/00-concepts.md#11-三个最常见的优化手段各在优化什么)）会失控。
...
2. **幂等键必须稳定标识一次工具调用。** 可从 `(workflow_id, step_id, tool_use_id)` 等稳定身份派生，也可首次生成随机 UUID 后和 checkpoint 一起持久化；不能在每次重试时生成新 key，否则幂等就退化成“重新执行一次”。
...
恢复能保住多少进度取决于引擎、图结构和 checkpoint 边界。把长节点拆成有明确输入输出的小步骤通常能缩小重做范围；拆成更多 Agent 本身不保证恢复性，还会增加协调、权限和一致性成本。
...
| 对象 | 写者模型 | 机制 |
|---|---|---|
| **会话上下文** | **单写者（single-writer）** | 一个 session 同时只允许一个活跃 runner。租约记录带单调递增的 `lease_epoch`：首次获取或过期接管必须原子地递增 epoch；每次会话写都带上它，并由存储层用 `WHERE lease_epoch = ?` 拒绝旧 runner。TTL 只负责让新 runner 能接管，epoch 才能挡住 GC 暂停后醒来的旧 runner |
| **任务状态** | **单写者** | 与会话相同；接管先原子递增 `lease_epoch`，任务写同时校验 `owner_id + lease_epoch + version`，不能只做 `WHERE version = ?` |
...
这里不能把租约 TTL 当成互斥证明：旧 runner 可能暂停到租约过期，新 runner 已经接管后它才恢复。若写入语句不校验更高的 `lease_epoch`（即 fencing token），两个 runner 仍会同时写；“只有一个活跃租约”只是协调层的愿望，不是存储层保证。
...
- **`scope` 是强制查询条件，不是过滤器**。向量检索尤其危险——先做向量近邻再按 tenant 过滤，会在候选集里泄露信息（延迟差、结果数量差都是侧信道 side channel：攻击者读不到数据本身，但能从耗时、返回条数这类间接现象反推出数据存不存在），且 top-k 会被别的租户的向量挤占。**必须是先隔离再检索**：Pinecone 走 namespace、Weaviate 走原生 multi-tenancy、Qdrant 走 payload 索引 + 过滤（过滤在 HNSW 图遍历内部执行，见 [`02 §5`](02-context-engineering-and-rag.md#5-检索流水线全景)）、pgvector 走 RLS（row-level security，行级安全：数据库自己按行判断这一行该不该给当前用户看，绕过它需要改数据库权限而不是改一行应用代码）+ `tenant_id`。
...
5. **记忆写入没有 eval 门禁** —— 上线三个月后你会有一个装满矛盾事实的库，而且没有任何指标下降来告诉你。最低限度要监控记忆条数增长率、`use_count == 0` 的占比和 supersede 率；阈值必须从自身基线和人工抽检校准，不能把 `<30%` 当行业健康线。
...
| 指标 | 健康值 | 异常含义 |
|---|---|---|
| 记忆注入 token / 总输入 token | 从 2–5% 做实验起点 | 相对质量/成本基线持续上升，可能是注入策略失控 |
| `use_count == 0` 的记忆占比 | 建立分层基线 | 异常升高可能是写入判据过松，也可能是检索器漏召 |
...
1. **“记忆”是四类要求不同的状态被一个词绑架了，第一步是拆词而不是选存储。** 任务状态通常需要强一致和 CAS；会话编辑、用户刚修改/删除的偏好也可能需要版本控制或 read-your-writes。外部真相源保存引用或缓存即可，是否回源由权威性、新鲜度、延迟预算和可用性共同决定。
...
**AI 专章顺读下一篇** → [05-multi-agent-orchestration.md](05-multi-agent-orchestration.md)
