...
| 概念 | 一句话 | 详见 |
|---|---|---|
| 缓存的三条代价 | 数据会旧、多一个会挂的组件、失效逻辑是新的 bug 源 | [00-concepts §11](../00-foundations/00-concepts.md) |
| 索引在优化什么 | 用额外维护的结构，把"全部翻一遍"变成"直接定位" | [00-concepts §11](../00-foundations/00-concepts.md) |
| p99 / 尾延迟 | 排序后第 99% 位置的耗时；串成多跳之后会被放大 | [00-concepts §3](../00-foundations/00-concepts.md) |
| TTFT 与 prefill | 到第一个字的时间，主要由"读完你的输入"这一段决定 | [01 §1](01-llm-serving-infra.md) |
| 前缀缓存 | 请求开头完全相同的那段只算一次，后来者直接复用 | [01 §6](01-llm-serving-infra.md) |
...
根源是 Transformer 的 n² 结构：**注意力是有限预算**，每对 token 都要建立关系（注意力机制里任意两个 token 之间都要算一次相关度，所以窗口里有 n 个 token 就有 n² 对关系要算，长度翻倍代价变四倍）。你往窗口里塞的每一个 token，都在稀释其他 token 拿到的注意力份额。
...
2. **成本**：一次 100K token 请求仅 input 约 **$0.20**，1M token 约 **$2/次**；同一个问题走 RAG 的检索 query 约 **$0.00008** —— 差约 **1,250×**（⚠️ 二手估算，只当量级）。10 万 query/天：长上下文约 **$10,000/天** vs RAG **<$10/天**。
...
**这直接杀死了两个常见设计：** ①**动态工具列表** —— "根据当前任务动态挑选工具"听起来很聪明，实际上每次工具集变化都让整个前缀作废；正确做法是工具集按**会话**固定（或按任务阶段分段且边界稀疏），不按**步骤**变化。②**把检索结果放在系统提示后面** —— 每次检索结果不同 → 后面所有内容的缓存全废，检索结果必须放在**历史末尾**。
...
| 端到端 | faithfulness（有没有编）、answer relevance | LLM judge，κ ≥ 0.6 才可上线 |
...
- **LLM judge**：用另一个模型给答案打分。**κ**（Cohen's kappa）是它和人工标注的一致性系数，扣掉了"瞎猜也会蒙对"的部分；κ ≥ 0.6 才说明这个 judge 的分数值得信。
...
2. **上下文的排列顺序决定了它的价格，而不只是它的效果。** 稳定的东西（工具定义、系统提示、程序性记忆）必须在前且逐字节不变，每轮变化的东西（检索结果、工具输出）必须追加在末尾；把检索结果放到系统提示后面，等于每一轮都为整个窗口重新付一次全价。
...
**下一篇** → [03-agent-runtime.md](03-agent-runtime.md)
