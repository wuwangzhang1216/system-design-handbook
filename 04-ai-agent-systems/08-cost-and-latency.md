...
> Agent 的成本对**轮次是二次的**，不是线性的。所以第一顺位的优化从来不是"换个便宜模型"，
> 而是让前缀字节稳定（缓存命中）和让工具变粗（减少轮次）。
...
|---|---|---|---|---|
| 1 | **前缀字节稳定 + prompt caching** | 输入 −60~90%；TTFT −2~10× | 1–3 人日 | 静默 miss（无报错，只有账单）；多租户共享有泄露面（§12） |
| 2 | **修失败运行** | 直接 −10~15% | 数天（做归因） | 生产遥测：**14.2% 的总花费花在失败的运行上**（639,381 执行步，2026 上半年） |
| 3 | **上下文瘦身** | 总 token −30~84% | 1–2 周 | 压缩会击穿缓存；信息丢失导致重做反而更贵 |
| 4 | **轮次削减**（工具粒度重设计） | 二次放大：轮次 −40% ⟹ 成本 −54% | 1–3 周（改工具契约） | 工具"大而全"后模型选择变难 |
| 5 | **非关键路径降档**（子 agent 用 Sonnet/Haiku） | 该部分 −40~80% | 数天 | 无回归集则不知道降了什么 |
| 6 | **输出长度控制** | 输出 −20~50%；尾延迟明显改善 | 1–3 人日 | 截断导致重试，净成本上升 |
| 7 | **Batch API** | −50%（输入+输出），可与缓存叠加 | 数天 | 24h 窗口，用户在等的路径不适用 |
| 8 | **模型路由 / 级联** | −40~80%，保留 95~99% 质量 | 2–6 周 + 持续维护 | 路由器自身成本与错误率；**级联抬高 P99** |
| 9 | **语义缓存** | 命中部分 100% 省 | 1–2 周 | 语义相似 ≠ 答案相同；**必须按租户分区** |
| 10 | **蒸馏 / 微调** | 单位成本 −5~30× | $35k–120k + 数据 + 评测（⚠二手） | 上游迭代后不跟着变好，6–12 个月重做 |
| 11 | **自建 GPU** | 见 §9 盈亏平衡 | 数月 + 专职团队 $5k–15k/月 | 利用率 100%→20% 单位成本涨 5× |
...
| 持有费 | 无 | 无 | **显式缓存 $1.00 / 百万 token / 小时**（三家唯一） |
| 隔离域 | workspace（Bedrock/GCP 上按 organization） | 账户 | 项目 |
...
1. **失效是级联的**，渲染顺序 `tools → system → messages`：改 tool 定义 ⟹ 整条全废；改 system ⟹ system+messages 失效；改 `tool_choice` / 增删图片 ⟹ 仅 messages 失效。
...
**a) 工具输出（Δ 的主体，也是最大污染源）** —— 返回值走 schema 白名单，只回传真正需要的字段（`list_issues` 完整 JSON 8k token vs `[{id,title,state}]` 400 token）；文件读取强制分页（`offset`/`limit`）；大结果落盘只回句柄：`saved to /tmp/r42.json (1,240 rows, cols: ...)`。

**b) 工具定义（L₀ 的主体）** —— 30 个工具 × 400 token = 12,000 token，**每轮全量重发**。做法：按阶段/角色做工具的命名空间加载，别把所有 MCP server 的工具一次全挂上。
...
| 做法 | 说明 | 典型收益 |
|---|---|---|
| **批量参数** | `read_file(path)` → `read_files(paths[])`；`get_issue(id)` → `get_issues(ids[])` | 5 次调用变 1 次 |
| **一次给够** | 搜索工具返回 snippet 而非仅 ID，避免必然发生的第二次 fetch | 砍掉一整类往返 |
| **并行工具调用** | 一轮内发多个 `tool_use`。⚠ 注意 20-block 窗口；**并行只用于"读"和"评"，"写"必须单线程** | 轮次 −30~50% |
| **确定性工作出模型** | 分页、排序、字段过滤、格式转换、单位换算用代码做 | 消灭"模型算错再重试"的整轮 |
| **明确终止条件** | MAST 分类里"不知道终止条件"占 **12.4%**、"步骤重复"占 **15.7%** | 消灭长尾会话 |
...
Haiku 4.5 : Opus 5 的价格比约 1:5 ⟹ **升级率超过 80% 时，级联比直接用大模型还贵，而且更慢。** 上线前必须先离线量出 `p_escalate`；量不出来就别上。
...
**三条工程纪律**：① 路由决策做成可一键下线的旁路（feature flag），出问题时 30 秒退回全走大模型；② **先埋点后路由** —— 在全走大模型的状态下用影子流量跑小模型 + judge 对比，统计"多少比例小模型也能过"，低于 30% 就不值得做；③ **路由维度不只是模型**，还有 thinking budget、是否开工具、是否开检索。关掉 thinking 往往比换模型省得更多且质量损失更小 —— HAL 的 21,730 rollout 实验结论是**多数运行中提高 reasoning effort 反而降低准确率**。
...
- **禁复述**：系统提示里写死"不要复述工具结果、不要重复用户输入、直接给结论"。Agent 最常见的浪费是把刚读到的 200 行文件在回复里重抄一遍。
...
**算例**：单 H100 跑 70B FP8 + vLLM 连续批处理 ≈ **400 tok/s ≈ 34.5M tok/天**（⚠二手口径）。H100 现货 ~$2.00–3.00/hr，取 $2.50 ⟹ 裸租金 **$60/天**。
...
> 物理上限 34.5M tok/天（100% 利用率）。隐性倍数 3–5× 来自二手汇总：专职工程 $5,000–15,000/月、冗余副本、闲置时段、可观测性栈。

**利用率敏感性 —— 这张表就是自建决策的全部**（8×H100 跑 671B MoE，⚠二手，2026-06-17）：
...
**蒸馏 / 微调**同理。收益（⚠均为二手）：**5–30×** 单位成本下降；13B 微调在与前沿模型差 2 个准确率点内时，单 token 便宜 **12–40×**；单点案例给出投入 $35k–120k、回本约 **2.9 个月**。真实代价常被漏掉：**先得有评测体系**（LLM judge 起步就要 100+ 标注样本 + 每周维护）、几千到几万条训练对、**模型漂移**（上游迭代后你的蒸馏模型不跟着变好，6–12 个月要重做）、一整套新 serving 栈（见 [01-llm-serving-infra.md](01-llm-serving-infra.md)）、一个长期 owner。**判据：任务窄 + 量大 + 稳定 ≥ 6 个月，三者同时成立才做。** Agent 主循环通常不满足（提示和工具每周都在改）；适合蒸馏的是**边缘子任务**：意图分类、字段抽取、重排、query 改写、安全审查。
...
| 2 | **缓存感知路由（会话粘性）** | 极端对照 P90 TTFT **0.542 s**(precise) vs 31.083 s(approximate) vs **92.551 s**(random) ⟹ **57× / 170×** | 8 pod / 16×H100 / 150 租户 × 6k ctx。**这让 LLM 网关变成有状态路由** |
...
- **投机解码只降 decode 感知延迟，不降 TTFT**，且**并发下急剧衰减**：1.69×(c=1) → **1.05×(c=64)**；Qwen3-8B bs=2 的 1.93× → **bs=48 的 0.99×（反而变慢）**。高并发生产环境别指望它。
- **流式输出不改 wall-clock、不降成本，只改感知延迟。** 汇报成"延迟优化"会误导容量规划。
- **并行工具调用**（只读）、**预取**（模型还在生成时就对高置信下一步发起检索，命中白赚一整轮）、**投机执行**（小模型先跑候选、大模型确认后采用；**有副作用的动作绝不能投机**）、**把确定性工作挪出模型**（一个 200ms 的 SQL vs 一整轮 3s 的模型往返）。
...
**长尾：P99 用户可以是 P50 的 20 倍。** 这是所有按人头定价的 AI 产品的核心风险，放大来自三处**相乘**：
...
**成本分布是重尾的，均值毫无意义。** 定价用均值、容量规划用均值，两个都会翻车。行业背景：AI 产品平均毛利约 **52%**（ICONIQ 2026，n≈300）vs 传统成熟 SaaS **80–90%**；推理成本占总支出从 **20% 升到 23%**。
...
| 护栏 | 做法 | 为什么 |
|---|---|---|
| **配额按 $ 不按请求数** | `budget_usd_per_day` per tenant | 一个请求可以是 $0.001 也可以是 $5 |
| **per-run 硬上限** | `max_cost_usd` + `max_turns`，超了终止并返回已完成部分 | 防单次运行失控 |
| **降级阶梯** | Opus → Sonnet → Haiku → 排队 → 拒绝，**且对用户可见** | 静默降级会变成信任事故 |
| **定价对齐** | 订阅 + 超额按量 + 硬上限 | 纯订阅（行业 58%）不封顶重度用户；纯消费型（35%）用户不可预测 |
| 结果型定价参考 | Intercom Fin **$0.99/outcome**（一次会话只计一个，月最低 50）；Salesforce Agentforce Flex Credits **$0.005/credit**，三动作工单 ≈ $0.30 | 结果型占 18%，正在增长 |

计量管道的正确性（幂等、去重、对账）见 [03-saas-platform/02-billing-and-metering.md](../03-saas-platform/02-billing-and-metering.md)。
...
| 10 | **前缀缓存无条件收益** | 存在争议：一派实测吞吐 **+30–50%**，另一派实测 **−36.7%**、TPOT +25%。差别在前缀重叠率与并发数 —— **上线前必须先量出重叠率** |
