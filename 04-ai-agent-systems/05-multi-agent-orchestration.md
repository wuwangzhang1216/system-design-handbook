...
> 一条安全的默认规则是：纯读和相互独立的评估可以并行；写默认串行。只有资源所有权明确、冲突键不相交且下游幂等时，写才可以并行。
...
| 概念 | 一句话 | 详见 |
|---|---|---|
| Agent 循环 | "模型输出 → 调工具 → 把结果塞回上下文 → 再输出"这个反复的过程 | [03 §1](03-agent-runtime.md#1-agent-循环的最小正确实现) |
| 终止条件 | 这个循环凭什么停下来；不写清楚它是 Agent 系统第一大 bug 源 | [03 §2](03-agent-runtime.md#2-终止条件agent-系统的第一大-bug-源) |
| 上下文腐化 | 窗口里塞得越多，模型对其中每一条的注意力越差 | [02 §2](02-context-engineering-and-rag.md#2-上下文会腐化context-rot这一节的实证决定了后面所有设计) |
| 前缀缓存 | 请求开头逐字节相同的那一段可以复用上次的计算，价格降到约 1/10 | [01 §6](01-llm-serving-infra.md#6-前缀缓存与缓存感知路由网关设计的真正约束) |
| 扇出与尾延迟放大 | 一个请求分发给 N 个后端并等全部返回时，整体 p99 逼近单后端的更高分位 | [00/01 §7](../00-foundations/01-fundamentals.md#7-尾延迟放大tail-latency-amplification) |
| 幂等 | 同一个操作执行多次，效果和执行一次相同 | [00/01 §5](../00-foundations/01-fundamentals.md#5-幂等idempotency分布式系统的第一公民) |
...
一次中等复杂度的研究任务，单 Agent 约 200K 输入 + 20K 输出（Opus 5 档，$5/M in、$25/M out）
  ...
  多 Agent 的任务尾部通常不同；若系统提示、工具定义等公共前缀逐字节一致，它们仍可共享公共前缀缓存
```

**注意最后一行**：多 Agent 不只是 token 多，worker 的任务尾部和历史通常彼此分叉，因而可复用比例可能低于单会话。不过 prefix cache 复用的是逐字节相同的前缀：统一的系统提示、冻结的工具定义和公共政策仍能共享，父级动态历史则通常不能直接复用。真实倍数应以 token 用量、缓存命中和成功任务成本实测，不能由“多 Agent”结构直接推出。
...
| **Map-Reduce 扇出（fan-out：把一份工作同时分发给 N 个执行者，见 [00/01 §7](../00-foundations/01-fundamentals.md#7-尾延迟放大tail-latency-amplification)）** | 同构任务 N 份并行 → 单点 reduce | ~N× 计算，但墙钟时间（wall-clock time，真实流逝的秒表时间） −最多 90% | 任务**完全同构且互不依赖**：批量文档抽取、批量文件分类 | reduce 阶段上下文爆炸；分片边界（shard boundary）跨越了语义边界 |
...
| **你还没有轨迹级（trajectory-level）eval** | 轨迹（trajectory）= 一次运行里模型的每一步输出、每一次工具调用及其返回值组成的完整序列；只看最终答案对不对叫 outcome-only。多 Agent 的失效是隐蔽的（轨迹脏但答案对、答案对但花了 10 倍钱）。**outcome-only eval 对多 Agent 是全盲的**（评测分层见 [06 §2](06-evaluation-and-observability.md#2-评测金字塔eval-pyramid)） |

> **面试金句**：
> "我不会先问‘要不要多 Agent’，我会先问任务之间是否有共享写资源。默认架构是：N 个只读 worker 并行探索 → 一个 lead 综合 → 一个 writer 落盘 → 一个确定性验收器判定。若写入资源能用稳定冲突键证明互不相交，并且副作用幂等，才放开受限的并行写。"
...
- 一项特定基准观察到 orchestrator 约在 **6 步**后出现熵饱和；同一研究里某模型 Step1 → Step2 的成功率从 **92% 掉到 22%**，不同模型的任务成功率区间 **7.43% ~ 44.14%**。这是需要在自身任务上验证的风险信号，不是通用“第 6 步定律”。
...
3. 编排步骤持续增加且质量/成本曲线出现拐点时，切成分层、脚本化或外部状态机；可先把 6 步当实验检查点，但必须用自己的轨迹数据确定阈值。
...
**工程推论有两条**：
1. checkpoint 应落在昂贵或有副作用的边界；把长节点拆成小的、可独立重放的步骤通常能缩小重做范围，但不必每步都变成新 Agent。
2. 并行度越高，“停在中间”的重跑代价可能越大；实际取决于引擎是否逐分支记录完成状态。

同时注意运行时约束：脚本本身**无文件系统 / shell 访问**（只有 Agent 有）；运行中**不能插入用户输入**（只有权限提示能暂停）；生成的 subagent 一律以 `acceptEdits` 模式运行并继承你的工具 allowlist（白名单：只有列在名单里的命令/工具才允许执行，名单外一律拒绝，见 [07 §5.1](07-agent-security.md#51-第一位能力最小化--从-least-privilege-到-least-agency)） ⇒ **长跑前必须把需要的命令预先加入 allowlist**，否则会在半路卡死。
...
**先定义 span**：一次调用在链路追踪里的一段记录 —— 有开始时间、结束时间、一组属性，以及"我的父亲是谁"。多个 span 按父子关系组成一棵树，这棵树叫一条 trace。下面这棵树就是一次多 Agent 运行的完整形状（span 树的完整设计规则在 [06 §6](06-evaluation-and-observability.md#6-可观测span-树设计)）。
...
**关键做法：可判定的轨迹指标用代码 evaluator，不要用 LLM judge**（LLM judge：用另一个模型按一份写死的评分标准给输出打分，代替人工标注；它自己的成本和偏差见 [06 §4](06-evaluation-and-observability.md#4-llm-as-judge怎么做才不骗自己)）。 布尔轨迹分数在每条采样 trace 上计算几乎是免费的，适合直接做告警信号；只有语义层（"这一步选这个工具合不合理"）才交给 judge。轨迹匹配可以做成四档：`strict`（完全一致同序）/ `unordered`（同一组工具，顺序不限，适合并行取数）/ `subset`（不许多调）/ `superset`（至少覆盖参考轨迹）。
...
1. **“Anthropic 说好、Cognition 说不好，二选一”** —— 两边讨论的是不同象限（只读研究 vs 有依赖的写入）。安全默认是“读可并行、冲突写串行”，无冲突写可在幂等与资源隔离成立时并行。
...
1. **读通常可并行，写默认串行**。只有资源所有权明确、冲突键不相交并且副作用幂等时，写才适合并行；否则语义冲突往往到合并后才暴露。
...
3. 在引用研究的任务分布里，编排者承担了 67.7% 的失败责任，因此应优先检查拆解、上下文和终止策略。推理档位与最大步骤数都要用轨迹 eval 调参，不能把“低档位、6 步”当所有场景的固定答案。
...
**AI 专章顺读下一篇** → [06-evaluation-and-observability.md](06-evaluation-and-observability.md)
